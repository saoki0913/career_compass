...
if printf '%s' "$PROMPT" | grep -qE 'src/components/|page\.tsx|layout\.tsx|loading\.tsx|UI|LP|デザイン|見た目'; then
  CONTEXT="${CONTEXT}UI 系依頼です。ui-designer agent を優先してください。設計判断が必要なら npm run ui:preflight、変更後は npm run lint:ui:guardrails と npm run test:ui:review -- <route> を推奨します.\n"
