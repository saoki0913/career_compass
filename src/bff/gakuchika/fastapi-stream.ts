...
import type { Identity } from "@/bff/gakuchika/access";
...
const FASTAPI_GAKUCHIKA_STREAM_TIMEOUT_MS = 60_000;
  ...
  for await (const { event, telemetry } of readSSEDataEvents(body)) {
    ...
    if (type === "string_chunk" && event.path === "question" && typeof event.text === "string") {
      ...
      const state = data.conversation_state
        ? safeParseConversationState(data.conversation_state)
        : buildConversationStatePatch(getDefaultConversationState(), partialState);
