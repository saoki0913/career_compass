...
function CompanyKanbanColumnComponent({
  ...
  return (
    <section
      ...
      className={cn(
        "flex min-h-[34rem] w-[19.5rem] shrink-0 flex-col rounded-xl border border-border/70 bg-muted/20 p-2 transition-colors",
        "sm:w-[20.5rem] xl:w-auto xl:min-w-0 xl:flex-1",
        isOver && "border-primary/40 bg-primary/5 ring-2 ring-primary/20"
      )}
    >
      <div className={cn("flex h-10 items-center gap-2 rounded-lg px-3", COLUMN_HEADER_COLORS[phase.color])}>
        <h2 className="min-w-0 flex-1 truncate text-sm font-bold">{phase.label}</h2>
      ...
      <div className="mt-2 flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto pr-0.5">
        ...
        {companies.length === 0 ? (
          <div
            className={cn(
              "flex min-h-32 flex-1 flex-col items-center justify-center rounded-lg border border-dashed border-border/80 bg-background/60 px-4 py-6 text-center text-xs text-muted-foreground",
