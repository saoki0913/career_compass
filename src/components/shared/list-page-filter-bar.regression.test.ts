...
describe("ListPageFilterBar regressions", () => {
  ...
  it("keeps controls and tabs inside a single horizontal scroll row", () => {
    ...
    expect(source).toContain("flex w-max min-w-full flex-nowrap items-center");
    expect(source).toContain("overflow-x-auto pb-1 [-webkit-overflow-scrolling:touch]");
    expect(source).not.toContain("space-y-3");
  });

  it("keeps the skeleton aligned to the single-row filter bar", () => {
