...
export const tasks = pgTable(
  "tasks",
  {
    ...
    sortOrder: integer("sort_order").default(0),
    /** Task dependency: ID of the task that must be completed before this one */
    dependsOnTaskId: text("depends_on_task_id").references((): AnyPgColumn => tasks.id, { onDelete: "set null" }),
    /** Whether this task is blocked by an incomplete dependency */
    isBlocked: boolean("is_blocked").notNull().default(false),
    /** Template origin key (e.g. "interview.research") for traceability */
  ...
  (t) => [
    ...
    index("tasks_depends_on_task_id_idx").on(t.dependsOnTaskId),
  ]
);
...
export const calendarSyncJobs = pgTable(
  ...
  (t) => [
    ...
    // NOTE: partial index calendar_sync_jobs_pending_scheduled_idx is managed via raw SQL migration (CONCURRENTLY, non-transactional)
  ]
);

// Task templates table - category-based task generation templates
export const taskTemplates = pgTable(
  "task_templates",
  {
    id: text("id").primaryKey(),
    /** Template category: maps from deadline types via DEADLINE_TYPE_TO_TEMPLATE_CATEGORY */
    category: text("category", {
      enum: ["es_submission", "test", "interview", "briefing", "internship", "offer_response"],
    }).notNull(),
    title: text("title").notNull(),
    taskType: text("task_type", { enum: ["es", "web_test", "self_analysis", "gakuchika", "video", "other"] }).notNull(),
    sortOrder: integer("sort_order").notNull().default(0),
    /** Days before deadline that this task should be due */
    daysBeforeDeadline: integer("days_before_deadline").notNull().default(0),
    /** Sort order of the task this depends on (null = no dependency) */
    dependsOnSortOrder: integer("depends_on_sort_order"),
    /** System-managed template (not user-editable) */
    isSystem: boolean("is_system").notNull().default(true),
    createdAt: timestamptz("created_at").notNull().defaultNow(),
  },
  (t) => [
    index("task_templates_category_idx").on(t.category),
    index("task_templates_category_sort_order_idx").on(t.category, t.sortOrder),
  ]
);
...
export const motivationConversations = pgTable(
  "motivation_conversations",
  {
    ...
    questionCount: integer("question_count").notNull().default(0),
    status: text("status", { enum: ["in_progress", "completed"] }).default("in_progress"),
    motivationScores: jsonb("motivation_scores").$type<unknown>(),
    generatedDraft: text("generated_draft"),
    charLimitType: text("char_limit_type", { enum: ["300", "400", "500"] }),
    conversationContext: jsonb("conversation_context").$type<unknown>(),
    ...
    lastEvidenceCards: jsonb("last_evidence_cards").$type<unknown>(),
    stageStatus: jsonb("stage_status").$type<unknown>(),
    feedbackSummary: jsonb("feedback_summary").$type<unknown>(),
    createdAt: timestamptz("created_at").notNull().defaultNow(),
    updatedAt: timestamptz("updated_at").notNull().defaultNow(),
  },
  (t) => [
...
export const interviewConversations = pgTable(
  "interview_conversations",
  {
    ...
    questionCount: integer("question_count").notNull().default(0),
    stageQuestionCounts: jsonb("stage_question_counts").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
    completedStages: jsonb("completed_stages").$type<string[]>().notNull().default(sql`'[]'::jsonb`),
