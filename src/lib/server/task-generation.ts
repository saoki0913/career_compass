...
type TaskGenerationExecutor = Pick<typeof db, "insert" | "select">;

/**
 * Generate tasks for a deadline based on its type's template category.
 * Uses the provided executor so callers can include generation in a larger transaction.
 * Returns the IDs of the created tasks.
 */
export async function generateTasksForDeadlineWithExecutor(
  ...
  const existing = await executor
    ...
    .where(
      and(
        eq(tasks.deadlineId, params.deadlineId),
  ...
  await executor.insert(tasks).values(taskValues);
  return taskIds;
}
...
export async function generateTasksForDeadline(params: GenerateTasksParams): Promise<string[]> {
  ...
  return db.transaction(async (tx) => {
    return generateTasksForDeadlineWithExecutor(tx, params);
  });
}
