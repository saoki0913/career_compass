...
> Claude Code ハーネス全体（agents / skills / hooks / MCP / commands）の詳細リファレンスと運用ガイドは [`docs/operations/development/AI_HARNESS.md`](../operations/development/AI_HARNESS.md) を参照してください。本ドキュメントは MCP と Notion 取り込み手順のみを扱います。
...
Supabase / Railway / Vercel / GitHub / Stripe / OpenAI Developer Docs は MCP を導入せず、CLI または公式 docs 検索を直接利用する方針です。理由と許可 / 禁止操作は [`docs/operations/development/CLI_GUARDRAILS.md`](../operations/development/CLI_GUARDRAILS.md) を、全体方針は [`docs/operations/development/AI_HARNESS.md`](../operations/development/AI_HARNESS.md) 6.3 節を参照してください。`gh auth token` や OAuth token、provider secrets は config・ログ・handoff に残しません。
...
詳細手順は本節の手順を正本とします。旧 `NOTION_REFERENCE_ES.md` は廃止済みです。
...
- `docs/reference/es-review/notion_reference_es_import.json`
- `docs/reference/es-review/<template>.md`（人間レビュー後に抽象ヒント化）
...
- [`docs/operations/development/AI_HARNESS.md`](../operations/development/AI_HARNESS.md) — Claude Code ハーネス全体のリファレンス
- [`docs/operations/development/CLI_GUARDRAILS.md`](../operations/development/CLI_GUARDRAILS.md) — CLI ガードレール（MCP を導入しない CLI の運用ルール）
