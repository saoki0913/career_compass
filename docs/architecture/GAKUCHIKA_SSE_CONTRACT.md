...
| `paused_question` | `pausedQuestion` | `string \| null` |
...
### paused_question (2026-04-27)

`draft_ready` / `interview_ready` のように入力欄を閉じる状態でも、LLM が返した次の深掘り候補を消さずに会話内へ残すためのフィールド。

- **生成**: `backend/app/normalization/gakuchika_payload.py` で `server_ready` / `deepdive_complete` の complete snapshot に保存する。通常の `ask` 状態では `null` に戻す
- **送信方法**: Complete: `data.conversation_state.paused_question`
- **Next API の補完**: FastAPI payload に質問があるのに `paused_question` が欠ける場合、`next_action !== "ask"` の complete 処理で `qText` を `pausedQuestion` に入れて永続化する
- **Client 表示**: `shouldPauseConversation` で composer を閉じても、`nextQuestion || pausedQuestion` を assistant message として表示する。これにより CTA 表示時に LLM の質問が消えない
...
- `next_action !== "ask"` かつ `data.question` が存在する complete では、永続化 state の `paused_question` に同じ質問が残る
