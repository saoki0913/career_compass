...
status: 完了
based_on_review: docs/review/rag-architecture/2026-04-17-rag-design-review.md
---
...
当初残件（2026-05-01 closeout 済み）:

- P0-1 は closeout 済み。golden set 52 件、strict tenant 評価 corpus 再投入 CLI、baseline integrity gate、実検索 baseline を追加した。
- Grafana dashboard は `docs/ops/grafana/rag-dashboard.json` として追加済み。
- P2-1 の ingest CLI と参考 ES eval set は実装済み。feature flag `REFERENCE_ES_RAG_ENABLED=false` は安全側の既定として維持する。
- P2-2 は比較 runner まで実装済み。ctx collection が空のため `CONTEXTUAL_RETRIEVAL_ENABLED` の default-on は見送り、backfill 後の再評価に分離する。
...
- [x] `docs/ops/OBSERVABILITY.md` (新規) で metric 一覧と alerting ルールを文書化
- [x] Grafana dashboard の JSON を `docs/ops/grafana/rag-dashboard.json` に commit
...
- `backend/scripts/ingest_reference_es.py` を追加し、匿名化済み JSONL の dry-run / ingest を可能にした。
- `backend/evals/rag/golden/reference_es_v1.jsonl` に最小 eval seed を追加した。
- loader は `anonymized=true`、`usage_consent=true`、`anonymization_level`、`source_provenance` を必須化し、ingest 単位の `ingest_session_id` を metadata に保存する。
- `backend/evals/rag/evaluate_reference_es.py` を追加し、`--ingest` 付きで参考 ES retrieval eval を実行できるようにした。
- 2026-05-01 local eval: `n_items=3`, `Recall@10=1.0`, `nDCG@5=1.0`。最小 synthetic seed は gate を満たす。
- `backend/tests/rag_eval/test_reference_es_ingest.py` で JSONL validation、Chroma where filter、contextual 比較 helper、reference ES eval helper を固定した。
- `REFERENCE_ES_RAG_ENABLED=false` は維持する。下流 ES レビューへの prompt 接続と出力類似度漏洩ガードは別 gate として扱い、現時点で default-on しない。
...
- `backend/evals/rag/compare_contextual_retrieval.py` を追加し、同一 golden set を `CONTEXTUAL_RETRIEVAL_ENABLED=false/true` で比較できるようにした。
- 比較 runner は per-item 出力の `dense_retrieved_collections` から collection 分布を集計し、ctx collection が本当に使われたかを見える化する。
- 2026-05-01 full golden eval (`n_items=52`, `top_k=5`): disabled/enabled とも `nDCG@5(src)=0.8183959038`, `MRR(src)=0.8205128205`, `Hit@5(src)=0.8846153846`, `Recall(src)=0.8557692308`、delta は全て `0.0`。
- 同 eval の collection 分布は disabled / enabled とも `company_info__openai__text-embedding-3-small: 260`、`company_info__openai__text-embedding-3-small__ctx: 0`。local Chroma の ctx collection count も `0`。
- default-on 判断: **現時点では `CONTEXTUAL_RETRIEVAL_ENABLED` を ON にしない**。退行はないが ctx collection が空のため有効化評価として不十分。既存データの ctx backfill か dual-write ingest 後、ctx collection 分布が実際に出る状態で再評価してから切り替える。
...
- 2026-05-01: P0-3 docs / Grafana dashboard、Reference ES ingest CLI / seed / eval runner、Contextual Retrieval 比較 runnerを追加。Reference ES 最小 eval は PASS。Contextual Retrieval は full eval の主要指標 delta 0 だが ctx collection が空のため default-on しない判断で close。
