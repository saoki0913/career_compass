...
**#4 Billing final local hardening (2026-05-14)**:
- 本番前最終改善の今回スコープは **local improvement + commit** まで。staging / production deploy / push は含めない。
- Drizzle の transaction-based migrator で `CREATE INDEX CONCURRENTLY` が失敗し得る問題は、release migration runner 側で `CONCURRENTLY` を含む migration を non-transactional apply path に分岐させて根本対応する。これにより既存 `0031_perf_cost_indexes.sql` と Billing `0037_billing_reconciliation_state.sql` の両方を同じ apply model で扱う。
- Billing UX/API は、`past_due` / `unpaid` / `paused` / `incomplete` の有料ユーザーを checkout 409 dead-end に送らず Customer Portal へ誘導する。Checkout idempotency key は `userId + customerId + plan + period + timeBucket` に戻し、異なる plan / period の 10 分内衝突を防ぐ。
- Stored Stripe customer の deleted / owner mismatch は catch-all 500 ではなく structured 409 / 403 として返す。Portal return 後の `/api/settings/profile` 再取得失敗も structured error として UI に表示する。
- Direct-consume 系と reservation confirm の P1 money-integrity edge は、今回 reservation timing を変更せず、成功後 consume / confirm 不成立を構造化ログで顕在化する。upstream 前予約への変更は挙動影響があるため別承認タスクに分離する。
- 本番リリース前の手作業: Stripe Dashboard の Commerce Disclosure / business details / statement descriptor / webhook events 8 種確認、Vercel staging secret bundle の `VERCEL_PROJECT_ID` 補正、`node scripts/release/run-migrations.mjs --env production --dry-run --json`、`make stripe-preflight`、`make ops-release-check`、staging deploy 後の health 確認、production promotion 承認。
...
### 追加 RFC（環境分離 / backend config 統合）— 2026-05-18 追記

19 カテゴリとは独立して、環境変数 SSOT 整備調査から派生した 2 件の設計記録（RFC / 改善計画）を登録する。いずれも本タスク（環境変数 SSOT 整備）では設計のみで、実装は承認後の別タスクとする。

| RFC | ファイル | 性質 | 依存関係 | 推奨 Phase |
|-----|----------|------|----------|------------|
| Supabase 環境分離 | `docs/plan/completed/supabase-environment-separation-plan.md` | 実装記録（完了・`completed/` へアーカイブ済み）。staging は別 Supabase project `career-compass-staging`、production は `career-compass-db`。schema 分離案は Superseded | **`#16 Release` (`release-infrastructure-operations-plan.md`) の deploy / secrets pipeline と整合必須**。production DB migration 経路を staging deploy から切り離す | Phase 6（運用・DB 接続情報の反映は operator gate） |
| backend config 統合 | `docs/plan/backend-config-env-consolidation-improvement-plan.md` | 改善計画。backend `os.getenv` 直読み ~13 変数を `config.py` Pydantic Settings に統合（behavior-preserving 絶対） | **`#19 保守性` (`maintainability-clean-architecture-roadmap.md`) に従属**（設定集約・config.py 責務の局所改善）。`#18 パフォーマンス` (`performance-cost-optimization-plan.md`) の LLM cost lever（`LLM_*_JSON`）と解釈不変で整合 | Phase 6 Track B（#19 保守性 P1+ と同帯。独立性高く並行可） |

**ハード依存（追記分）:**
12. `#16 Release` deploy / secrets pipeline → Supabase staging project 反映（`supabase-staging` target と `.env.staging` migration runner を使う）
13. Supabase staging project 反映 → staging DB connection strings を secret bundle に投入してから migration / drift check 実行
14. `#19 保守性` の責務境界整理 → backend config 統合（config.py を肥大化させない原則に従属。ただし env 直読み解消は独立 PR 単位で先行可）

**配置理由**: Supabase 環境分離は本番 DB トポロジと release pipeline に触れるため Phase 6 帯に置く。別 project 分離により schema-aware FK 変換は不要になったが、secret bundle と staging migration runner の接続先誤りは production impact を持つ。backend config 統合は behavior-preserving な局所改善で独立性が高く、Phase 6 Track B（#19 と同帯）で並行可能。
...
9. **追加 RFC 2 件は Phase 6 帯（2026-05-18 追記）**: Supabase 環境分離（実移行は architecture gate + RFC 承認前提、#5 DB/RLS に従属）と backend config 統合（#19 保守性に従属、behavior-preserving 独立 PR 可）。本タスクでは設計確定のみ
