...
> **現状同期メモ（2026-05-13）**: LLM validation は `backend/app/services/es_review/llm_validation.py` と `_validate_rewrite_combined()` に実装済み。専用の文字数修復パスと互換 metadata は廃止済みで、文字数不足は focused retry の target window / delta band guidance で扱う。次の主戦場は `TemplateDef` の型安全化、ES review rewrite / validation / JSON repair への `AIOutboundPolicy` 適用、short-answer eval、prompt cache safety である。
...
- 専用の文字数修復パスは廃止（focused retry の target window + deterministic compression で代替）
...
- `execute_recovery_pipeline`: best-effort adoption / 422
...
#### 3.2.3 専用文字数修復パスの廃止 (2-C)

**対象ファイル**: `backend/app/services/es_review/orchestrator.py`

- `execute_recovery_pipeline()` から専用文字数修復ロジックを削除
- 文字数不足・小超過は focused retry の target window と deterministic repair に統合
...
**廃止理由**: 専用文字数修復は1回限りで length 以外の問題を修正しない。focused retry の target window と deterministic compression の組み合わせで十分カバーできる。
...
- `execute_recovery_pipeline()`: best-effort adoption ロジックを簡素化

---

### Phase 3: Prompt Structure Optimization — 実装完了

**Goal**: プロンプトの情報密度を最適化し、LLM の指示追従性を最大化する

**Status**: 実装完了（2026-05-13）

#### 実装サマリ

- **Dead code 削除**: `get_template_labels()`, `get_template_company_usage()`, `_dedupe_text_items` の重複を除去
- **重複関数統合**: `_format_template_evaluation_rubric`, `_format_template_required_elements`, `_format_template_anti_patterns` の3ペアをそれぞれ統合（optional `template_spec` kwarg で標準/フォールバック両対応）
- **RewriteStrategy enum 導入**: boolean `safe_mode` を `RewriteStrategy` enum（STANDARD / FALLBACK）に置換。`_StrategyConfig` dataclass で標準/フォールバック間の10項目の差分を一元管理
- **`build_template_fallback_rewrite_prompt` 簡素化**: 165行から薄いラッパーに縮小
- **6 compositor 関数作成**: `_format_length_section`, `_format_style_section`, `_format_template_section`, `_format_company_section`, `_format_context_section`, `_format_retry_section`
- **トップレベルセクション統合**: 約25セクションから9セクションに集約。ネストされたサブタグで LLM セマンティックハンドルを維持
- **安全な重複除去**: パターン #4, #5, #7 のみ除去。パターン #1, #2, #3, #6 は意図的な強調として維持（結論ファースト、だ/である、事実追加禁止、企業固有情報）
- **型安全化**: `TemplateDef` TypedDict に不足していた3キーを追加、`StyleRule` の Literal 型を強化
...
| `backend/app/services/es_review/orchestrator.py` | 2 | 専用文字数修復廃止 + retry loop 簡素化 |
| `backend/app/services/es_review/retry.py` | 2 | focused retry の target window 型安全化 |
...
| `_fit_rewrite_text_deterministically()` | `validation.py` | semantic compression |
...
- 専用文字数修復廃止後: deterministic compression と focused retry で char_max 内に収まることを assert (既存テストケース)
...
- 専用文字数修復廃止は Phase 2-C/2-D と同時に実施
