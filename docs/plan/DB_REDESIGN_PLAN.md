...
> 2026-05-01: DB-1/DB-2 を実施済み。`src/lib/db/relations.ts` を追加し、`src/lib/db/index.ts` で schema と relations を merge する構成に変更した。DB-4 の `gakuchika_conversations.messages` 二重 `JSON.stringify` は既に修正済みで、対象 route は配列を直接渡している。
>
> 2026-05-02: DB-3/DB-8 を実装済み。`drizzle_pg/0025_db_redesign_indexes.sql` に index migration を追加し、`deadline-loaders.ts` は owner/confirmed deadline に絞った `task_stats` derived table JOIN に変更した。`npm run db:generate` は既存 Drizzle snapshot drift の `calendar_sync_jobs` 対話プロンプトで通常生成できないため custom migration を採用。`npm run db:migrate:as-app` はローカル DB の migration 履歴ドリフトにより既存 `accounts` table 再作成で失敗するため、今回追加した index SQL はローカル DB に直接適用して検証した。
>
> 2026-05-02: DB-5〜DB-7 を実装済み。対象 10 カラムを `jsonb()` に変更し、`drizzle_pg/0026_db_redesign_jsonb_columns.sql` を追加した。ローカル DB では移行前 JSON 妥当性検証が全列 invalid=0、`0026` SQL 直接適用後に `information_schema.columns` で 10 カラムすべて `jsonb` を確認済み。read 側は `src/lib/db/jsonb-compat.ts` で旧 string / 新 jsonb の両対応にした。migration は malformed JSON の列別 preflight と app 互換 deploy → 旧 app instance drain → DB migration の適用順を明記し、write 側は `phase` / 通知設定 JSONB の形状検証を追加した。
...
- [x] AC-1: `src/lib/db/relations.ts` が存在する
- [x] AC-2: 上記の全リレーションが定義されている
- [x] AC-3: `npm run build` がエラー 0
...
- [x] AC-1: `db.query.users.findFirst({ with: { companies: true } })` が型エラーなしでコンパイルされる
- [x] AC-2: `npm run build` がエラー 0
- [x] AC-3: 既存の `db.select()` 系クエリと `createMissingDb()` の後方互換を型テストで確認
...
- `npm run db:generate` は既存 snapshot drift で `calendar_sync_jobs` の対話プロンプトが出るため、DB-3 では `drizzle-kit generate --custom --name db_redesign_indexes` で custom migration を作成した
- `npm run db:migrate:as-app` はローカル DB の Drizzle migration 履歴ドリフトで既存 `accounts` table 再作成に失敗するため、今回追加 SQL の構文・適用はローカル DB に直接適用して検証した
...
- [x] AC-1: custom migration `drizzle_pg/0025_db_redesign_indexes.sql` と `meta/0025_snapshot.json` を作成・同期済み
- [x] AC-2: 追加 index SQL はローカル DB に直接適用し、`pg_indexes` で存在確認済み
- [x] AC-3: EXPLAIN ANALYZE で `deadlines_company_open_due_idx` の Index Scan を確認済み
...
- [x] AC-1: `new/route.ts` に `JSON.stringify(initialMessages)` が存在しない
- [x] AC-2: `resume/route.ts` に `JSON.stringify(messages)` が存在しない
- [x] AC-3: `npm run build` がエラー 0
...
- [x] AC-1: 5 カラムすべてが `schema.ts` で `jsonb()` 型になっている
- [x] AC-2: 対象ファイルに保存用の不要な `JSON.stringify()` / `JSON.parse()` が存在しない（比較用と legacy fixture は除外）
- [x] AC-3: `npm run build` がエラー 0
- [x] AC-4: 対象 route / loader の unit と local DB migration 検証が正常
...
- [x] AC-1: deadline-loaders が 1 クエリで deadline + task 集計を取得する
- [x] AC-2: EXPLAIN で相関サブクエリが存在せず、`tasks_deadline_status_idx` の Index Only Scan を確認済み
- [x] AC-3: `/dashboard` 系 loader unit と `npm run build` が正常
- [x] AC-4: `npx tsc --noEmit` がエラー 0
