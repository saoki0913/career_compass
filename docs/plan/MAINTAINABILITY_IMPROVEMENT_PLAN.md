...
status: M系 + M-1b + CA-0〜CA-5 完了 (2026-05-02)
...
**現状の数値サマリー (2026-04-26 実測)**:

| 項目 | 実測値 | 目標 | 状態 |
|------|--------|------|------|
| motivation page.tsx | 44 行 | 50 行以下 | **M-4 完了** |
| interview page.tsx | 38 行 | 50 行以下 | **M-4 完了** |
| gakuchika page.tsx | 44 行 | 50 行以下 | **M-4 完了** |
| MotivationConversationContent.tsx | 499 行 | 存在 | **M-4 完了** |
| InterviewPageContent.tsx | 385 行 | 存在 | **M-4 完了** |
| GakuchikaConversationContent.tsx | 429 行 | 存在 | **M-4 完了** |
| useMotivationViewModel.ts | 240 行 | 存在 | **M-5 完了** |
| useInterviewViewModel.ts | 66 行 | 存在 | **M-5 完了** |
| useGakuchikaViewModel.ts | 78 行 | 存在 | **M-5 完了** |
...
| `src/lib/interview/persistence.ts` | 200 行 (実体) | 実体 | **M-2 完了** |
...
| 新規作成 | `src/components/gakuchika/GakuchikaConversationContent.tsx` | -- | -- | UI 本体の移動先 |
...
- `src/components/gakuchika/GakuchikaConversationContent.tsx` を新設
- `page.tsx` からインライン定義のコンポーネントとメイン関数を移動
- `page.tsx` を 50 行以下に書き換え
...
- [x] 3 つの page.tsx が各 50 行以下である (motivation 44, interview 38, gakuchika 44)
- [x] 3 つの Content component が存在し、UI 本体を包含する
- [x] `npx tsc --noEmit` が PASS する
- [x] 各画面の遷移、会話進行、完了サマリー表示、ナビゲーションガードが動作する
...
| gakuchika | `createSSEProxyStream` + `readSSEDataEvents` (旧 `iterateGakuchikaFastApiSseEvents` 廃止済み) | `stream-pipeline` + config |
| interview | `createInterviewUpstreamStream` wrapper 内で `createSSEProxyStream` 使用 | route support module の整理 |

**2026-04-26 実装メモ**: Gakuchika の通常 stream route は `createSSEProxyStream` 使用済み。初回/再開質問生成で使う `src/app/api/gakuchika/fastapi-stream.ts` も `src/lib/fastapi/sse-proxy.ts` の共通 `readSSEDataEvents` に移行し、旧 `iterateGakuchikaFastApiSseEvents` は削除済み。

**2026-04-26 型チェック復旧メモ**: `npx tsc --noEmit` の既存型エラーを解消。主な修正は Vitest globals 型の明示、`.mjs` tooling helper の JSDoc 入力型追加、古い test fixture の現行 contract 追従、`process.env.NODE_ENV` 直接代入テストの `vi.stubEnv` 化。
...
**Step 3: gakuchika stream route の統一（完了済み）**

- `route.ts` は `createSSEProxyStream` 経由に移行済み
- 初回/再開用の独自 `iterateGakuchikaFastApiSseEvents` ループを廃止し、共通 `readSSEDataEvents` を使用
...
**M-1a (transport統一) — 完了**:
- [x] Gakuchika の旧 `iterateGakuchikaFastApiSseEvents` parser が 0 件
...
| Gakuchika 旧 parser 0 件 | `rg "iterateGakuchikaFastApiSseEvents" src/app/api/gakuchika src/lib/fastapi` | 0 件 |
...
- [x] route 配下 (`src/app/api/`) に domain logic の実体 (serialize/deserialize/normalize/save) が残っていない
- [x] `src/lib/interview/` 内の各ファイルが 300 行以下 (context-builder 120行, persistence 200行)
- [x] `src/lib/motivation/conversation.ts` が型定義 + re-export のみ (300 行以下)
- [x] 契約テスト 3 ファイルが PASS
...
- [x] `llm.py` が 1,000 行以下 (986行)
- [x] `llm.py` の re-export ブロック (backward compatibility) が 0 行 (L35-64 は proper import に変換済み)
- [x] `cd backend && python -m pytest` 全 PASS (M-3 関連テスト全通過。pre-existing failures 2件は無関係)
- [x] `FASTAPI_MODULE_LAYOUT.md` が存在する
...
| 変更 | `src/components/gakuchika/GakuchikaConversationContent.tsx` | (M-4 で新設) -> 縮小 |
...
- 実ファイルである `GakuchikaConversationContent.tsx` から業務 state を移動
- gakuchika 固有: STAR 進捗判定、draft ready 条件、restart 判定

#### 4.5.4 受入基準

- [x] `MotivationConversationContent.tsx` が 500 行以下
- [x] `InterviewPageContent.tsx` が 600 行以下
- [x] content component 内に `useState` で管理する業務 state が残っていない (UI-only state は許容)
- [x] 3 つの view model hook ファイルが存在する
- [x] 全会話フローが正常動作 (E2E)
...
**ステータス:** 完了 (2026-05-02)。CA-1A backend は `backend/app/services/motivation/` へ service 層を移設し、`backend/app/routers/motivation.py` を 11 行の facade にした。CA-1B は `src/bff/motivation/` に Next API route 実体を移し、旧 `src/app/api/motivation/**/route.ts` を thin facade にした。CA-1C は `src/features/motivation/` に application / domain / hooks / ui の移行入口を追加し、product page と主要 UI が feature entrypoint を参照する形にした。CA-0 契約テスト再実行と motivation Playwright functional も PASS。
...
- [x] `routers/motivation.py` が 200 行以下
- [x] `src/features/motivation/` が存在
- [x] `src/bff/motivation/` が存在
- [x] CA-1A backend targeted pytest / architecture test / compileall が PASS
- [x] CA-1B/CA-1C targeted Vitest と `npx tsc --noEmit` が PASS
- [x] CA-0 の契約テストが PASS
- [x] Playwright の motivation フロー（guest / logged-in / draft 成功・失敗）が全 PASS
...
**ステータス:** 完了 (2026-05-02)。backend は `backend/app/services/es_review/` へ helper 実体を移し、既存 `backend/app/routers/es_review_*.py` は import 互換 shim にした。frontend/BFF は `src/bff/es-review/` と `src/features/es-review/` を追加し、旧 `_services` は re-export shim にした。境界テスト、targeted ES review tests、Playwright ES 添削回帰、`npx tsc --noEmit`、`npm run lint` が PASS。
...
- [x] `backend/app/services/es_review/` が存在
- [x] `src/features/es-review/` が存在
- [x] `handle-review-stream.ts` が bff 層に分離され、200 行以下
- [x] lint / import boundary regression が motivation + es_review 配下で PASS
- [x] Playwright の ES 添削フローが全 PASS

---

### Task CA-3: gakuchika

**ステータス:** 完了 (2026-05-02)。`backend/app/services/gakuchika/`、`src/bff/gakuchika/`、`src/features/gakuchika/` を追加し、router / App Router API / legacy hooks は互換 shim 化した。CA-1/CA-2 と同じく段階移行を優先し、`backend/app/routers/gakuchika.py` 本体の完全 thin router 化は CA-5 の残骸整理・追加分割候補として扱う。
...
- [x] `backend/app/services/gakuchika/` が存在
- [x] `src/features/gakuchika/` が存在
- [x] lint が gakuchika 配下で PASS
- [x] Playwright gakuchika フローが全 PASS

---

### Task CA-4: company_info 特殊扱い

**ステータス:** 完了 (2026-05-02)。backend は `backend/app/services/company_info/` を追加し、schedule fetch / RAG source build / deadline extraction を service 層へ分離した。既存 router helper は互換 shim 化し、router 層から service 層への依存方向を境界テストで固定した。frontend は `src/features/company-info/` へ企業情報 UI を移し、旧 `src/components/companies/CorporateInfoSection.tsx` と配下 modules は feature entrypoint への shim にした。
...
- [x] `backend/app/services/company_info/` が存在し、3 サブタスク (CA-4a/b/c) が完了
- [x] `src/features/company-info/` が存在
- [x] Playwright 企業情報取得 + 選考スケジュール取得フローが PASS（認証必須ケースはローカル auth state 不在により skip）

---

### Task CA-5: 全体 lint + docs 同期

**ステータス:** 完了 (2026-05-02)。`eslint-plugin-boundaries` と `import-linter` を導入し、`npm run lint:architecture` で frontend / backend の境界 lint を検証する。`CLAUDE.md` / `AGENTS.md` / `docs/architecture/ARCHITECTURE.md` / `BFF_FASTAPI_CONTRACT.md` / `FASTAPI_MODULE_LAYOUT.md` を CA-1〜CA-5 後の新構成へ同期し、旧 `src/lib/api-route/`、`src/app/api/_shared/`、`src/hooks/es-review/` の実コードを削除した。
...
- [x] lint (import-linter + eslint-plugin-boundaries) がリポジトリ全体で PASS
- [x] `CLAUDE.md` / `AGENTS.md` / `ARCHITECTURE.md` が新構成を反映
- [x] 旧ディレクトリの残骸コードが削除済み
...
- [x] 独自 SSE パース実装が 0 件 (全 feature が `createSSEProxyStream` / `createConfiguredSSEProxyResponse` 経由)
- [x] route 配下 (`src/app/api/`) に domain logic の実体が残っていない (re-export シム削除済み)
- [x] `llm.py` <= 1,000 行 (986行)、re-export ブロック 0 行 (proper import 化済み)
- [x] 全 product page (motivation / interview / gakuchika) が 50 行以下の thin wrapper
- [x] 3 つの Content component (MotivationConversationContent / InterviewPageContent / GakuchikaConversationContent) が存在する
- [x] content component 内の業務 state が view model hook へ移動済み
- [x] `FASTAPI_MODULE_LAYOUT.md` が存在する
- [ ] `npm run test:unit` / `npm run test:e2e` / `cd backend && python -m pytest` 全 PASS (Phase 4 で検証予定)
