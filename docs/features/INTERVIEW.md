...
| UI Helpers | `src/lib/interview/ui.ts` | `labelTopic()` / `labelWeakestQuestionType()` / progress narrative / `buildInterviewTopicStages()` / `buildInterviewPhases()` |
...
| Sheet Builder | `src/lib/interview/sheet-builder.ts` | 面接確認シート Markdown 生成 |
| BFF Sheet | `src/app/api/companies/[id]/interview/sheet/route.ts` | 面接確認シート生成・保存 |
...
| `complete` | 最終結果 | `{data: {question, turn_state, turn_meta, plan, next_question_hint, ...}, internal_telemetry}` |
| `error` | エラー | `{message}` |
...
### 3.6 質問数制御（12-18 問ソフト目標）

質問数はソフトミニマム 12 問 / ハードマックス 18 問で制御する。`planning.py` の定数:

| 定数 | 値 | 説明 |
|---|---|---|
| `QUESTION_SOFT_MIN` | 12 | 全 must_cover_topics covered かつこの問数以上で終了可能 |
| `QUESTION_HARD_MAX` | 18 | 無条件終了 |

`_should_end_questions(budget, plan)` の終了判定:

1. **Hard max**: `turnCount >= 18` → 必ず終了
2. **Soft min + coverage 完了**: `turnCount >= 12` かつ `must_cover_topics` が全て covered → 終了
3. **全 exhausted**: 全 `coverage_state` が `covered` / `exhausted` かつ `turnCount >= 6` → 終了

終了時は `_generate_turn_progress()` が LLM 呼び出しをスキップし、`question_flow_completed: true` + `question: ""` + `nextAction: "feedback"` で SSE complete を返す。クレジットは消費しない。

turn プロンプトには `question_budget` セクションで現在の問数・目標・残りトピックを注入し、LLM のトピック配分を誘導する。

### 3.7 回答ヒント（`next_question_hint`）

`next_question_hint` は生成した質問に対する回答のポイント（30-60 字）を提供する新フィールド。`short_coaching.next_edit`（前回回答のギャップ分析）とは異なり、**次の質問内容に直接連動**する。

- LLM が `INTERVIEW_TURN_SCHEMA` の `next_question_hint` フィールドで返す
- LLM が返さなかった場合は `_fallback_next_question_hint(turn_meta)` が `followup_style` に応じた deterministic ヒントを生成
- SSE complete の `data.next_question_hint` でフロントエンドに送出
- フロントでは `reducers.ts` が `kind === "send"` 時のみ `nextQuestionHint` を更新（start/feedback/continue 時は維持）
- UI 表示は `InterviewPageContent.tsx` で「回答のヒント: {nextQuestionHint}」として表示

`short_coaching.next_edit` は内部 state として残るが UI 表示は廃止済み。

followup_style ごとの fallback ヒント例:
...
4. **turns**: `POST /stream` で回答送信 -> 次質問生成。coverageState マージ、short_coaching 付与、`next_question_hint` 生成、question 正規化（`_normalize_question_text()`）。質問数が budget 上限（soft min 12 / hard max 18）に達した場合は LLM 呼び出しをスキップし `question_flow_completed: true` で終了
...
- **short_coaching**: `_fallback_short_coaching()` が answer_gap に応じた `good` / `missing` / `next_edit` の 3 フィールドを機械的に生成。LLM が返さない場合のfallback。UI 表示は `next_question_hint` に移行済み
- **next_question_hint**: `_fallback_next_question_hint()` が `followup_style` に応じた回答ヒントを機械的に生成。LLM が `next_question_hint` を返さなかった場合の fallback
- **質問数制御**: `_should_end_questions()` が soft min (12) / hard max (18) / coverage 完了を判定し、`_generate_turn_progress()` で LLM 呼び出し前に終了チェック
...
`_generate_turn_progress()` は LLM 呼び出し**前**に `_should_end_questions()` を評価し、終了条件を満たす場合は LLM をスキップして `question_flow_completed: true` + `question: ""` で SSE complete を返す（クレジット非消費）。
...
講評完了後に「面接確認シートを保存」ボタンで Q&A 全文と評価をまとめた Markdown シートを生成・保存する機能。

**生成フロー**:
1. ユーザーが講評表示後のボタンをクリック
2. `saveInterviewSheet(companyId, conversationId, historyId)` が `POST /api/companies/[id]/interview/sheet` を呼び出し
3. BFF が `interviewConversations` から messages、`interviewFeedbackHistories` から評価データ、`companies` から企業名を取得
4. `buildInterviewSheetMarkdown()` で Markdown を生成
5. `saveInterviewFeedbackSheet()` で `interviewFeedbackHistories.sheetContent` / `sheetGeneratedAt` に保存
6. 冪等: 既に `sheetContent` がある場合は再生成せず既存内容を返却

**シート構成** (`src/lib/interview/sheet-builder.ts`):
- ヘッダー（企業名 + 生成日時）
- 面接設定（方式 / 選考種別 / 段階 / 面接官 / 厳しさ / 職種）
- Q&A 全文（番号付き質問-回答ペア）
- 7 軸スコアテーブル
- 良かった点 / 改善点
- 一貫性リスク（存在時のみ）
...
**UI**: 保存成功時はトースト通知 + ボタンが「面接確認シート保存済み」に変化。

### 8.6 `labelWeakestQuestionType()` マッピング

`weakestQuestionType` の内部キーを日本語ラベルに変換する。`src/lib/interview/ui.ts` で定義。
...
- `sheetContent` / `sheetGeneratedAt` — 面接確認シート（Markdown）とその生成日時
...
| | `src/lib/interview/sheet-builder.ts` | ~100 |
| | `src/lib/interview/persistence*.ts` (5 files) | ~800 |
...
| | `src/app/api/companies/[id]/interview/sheet/route.ts` | ~178 |
...
| `POST` | `/api/companies/[id]/interview/sheet` | 面接確認シート生成・保存 |
