...
| 主モデル | `MODEL_ES_REVIEW`（例: `claude-sonnet`, `gpt-5.1`, `gemini-3.1-pro-preview`, `command-a-03-2025`, `deepseek-chat`） |
...
| 参考ES | `quality hints + coarse skeleton + overlap guard` にだけ使う |

標準経路は backend の既定値として `MODEL_ES_REVIEW` を持ちつつ、フロントの `モデル選択` dropdown から設問ごとに切り替えられる。現在 UI で選択可能なのは `Claude Sonnet 4.5` と `GPT-5.1` で、`Gemini 3.1 Pro Preview / Cohere Command A / DeepSeek V3.2` は shared path を維持しつつも live smoke が安定するまで `現在調整中` として無効化している。UI の既定選択は Claude Sonnet 4.5。Qwen3 Swallow 32B β は別 route からだけ呼び、入力防御、RAG、response shape は既存 ES 添削と揃えるが、prompt・context compaction・semantic validator は Qwen 専用実装を使う。

標準経路の shared provider layer では、Claude 専用 transport を変えずに OpenAI / Gemini / Cohere / DeepSeek の structured output 契約を揃える。

- OpenAI Chat Completions は `response_format.type=json_schema` に `name / schema / strict` を必ず付ける
- Gemini は `responseMimeType=application/json` と schema を使い、JSON 前置きが混ざっても最初の balanced object を優先して parse する
- Cohere / DeepSeek も同じ JSON validator と retry 契約で扱う

Qwen β の追加方針:

- short-answer では context を Claude より小さくする
- `post_join_goals` / `intern_goals` / `company_motivation` / `role_course_reason` は Qwen 専用の設問適合 validator を通す
- Qwen client は non-thinking mode と strict structured output を使う
- Qwen timeout は stage ごとに分ける
  - improvement JSON は短く待って timeout 時に即 fallback issues を使う
  - rewrite は長めに待ち、timeout 時は compact retry を 1 回だけ行う
  - compact retry も timeout したら deterministic な timeout fallback rewrite を返す
  - `review_meta` には `improvement_timeout_fallback / timeout_stage / timeout_recovered / rewrite_generation_mode` を載せる
...
| `prestream_source_urls` | その回の添削前に取得した corporate URL。current-run の evidence として優先注入する |
| `user_provided_corporate_urls` | ユーザーが手動追加した URL / PDF。最優先 evidence として使う |

### 企業情報ソースの優先順位

- `user_provided_corporate_urls`
  - 手動で追加した URL とアップロード PDF
  - 「参考にしてほしい」という明示意図があるため最優先で使う
- `prestream_source_urls`
  - その回の添削の直前に追加取得した企業ページ
  - BM25 更新待ちに依存せず、source URL 指定で current-run に直接差し込む
- 通常 RAG 検索結果
  - 既存 corporate RAG から relevance 順に取得する

この priority は ES 添削の evidence selection 用であり、企業情報取得機能の `official / high / medium / low` confidence 表示とは分けて扱う。
...
required 設問では、通常検索に加えて `prestream_source_urls` と `user_provided_corporate_urls` を current-run で直接注入する。これにより、直前に取得した社員インタビューやユーザーアップロード PDF が、その回の添削から evidence card に反映される。
...
- priority content types:
  ...
  - `corporate_site`
- `short_circuit=False`
- required template では `weak` だけでなく `partial` coverage でも、`役割/プログラム軸` と `企業理解軸` の片方が欠けていれば second pass を走らせる
- second pass は最大 1 回で打ち切り、追加の LLM 呼び出しは増やさない
...
- role 名そのものを深掘りしない
- 設問と回答から `事業理解 / 成長機会 / 価値観 / 将来接続 / 役割理解 / インターン機会` の軸を抽出する
...
改善ポイント生成では標準経路 provider 共通の JSON parse retry を 1 回だけ有効にしている。
...
- `current_answer` は必ず 1 件以上確保する
- `gakuchika_summary / document_section / gakuchika_raw_material` から補助 fact を最低 1 件は確保する
- `profile` は role/industry/company と重なるときだけ 1 件まで補助的に使う

設問軸の term も加点に使い、generic role のときは `事業理解 / 成長機会 / 価値観 / 将来接続 / 役割理解 / インターン機会` の軸と噛み合う事実を優先する。
`company_evidence_cards` は `rag_sources` から設問・職種・インターン名との一致度で選び、`theme / claim / excerpt` に圧縮する。

- `required` の設問では最大 4 件
- `assistive` の設問では最大 1 件
- `required` では `役割/プログラム系 theme` を最低 1 件、`企業理解/事業/価値観/将来接続系 theme` を最低 1 件確保する
- broad role では同一 theme の重複より theme diversity を優先する
...
- 通常 1 回目
  ...
  - 非Claudeの 300〜500 字帯 required 設問では、4文構成と `経験→役割→企業接点→価値発揮` の骨格を明示する
- compact retry
  - user facts 6 件まで
...
- length-focus retry
  - 非Claudeで `under_min` が続く場合、3 回目以降は「不足分を接続文で埋める」ことを主目的にした prompt へ切り替える
  - 新事実は足さず、既存の経験・職種・企業接点を 1 文増やして 390〜400 字へ寄せる
- fallback retry
  - user facts 5 件以下
...
- small miss length-fix
  ...
  - 非Claudeの 300〜500 字帯 required 設問では under-min の救済幅を 45 字まで広げる
...
標準モデルの 300〜500 字帯 required 設問では、under-min が大きめでも `length-fix` が入り、短い補足文で 390〜400 字に寄せる。
...
- UI で `Claude Sonnet 4.5 / GPT-5.1 / Gemini 3.1 Pro Preview / Cohere Command A / DeepSeek V3.2 / Qwen3 Swallow 32B β` のどれが結果を返したかを区別する
