...
`self_pr` では `経験不足` / `自信がない` のような自己否定語を narrow validator で検知し、focused retry では **事実は保ったまま、準備・責任感・学習姿勢・確認力などの前向きな行動特性へ言い換える**。
