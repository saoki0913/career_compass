...
本ドキュメントは ES Review 機能全体の SSOT である。プロンプトと rewrite pipeline の深掘りは `docs/prompts/es-review/ES_REVIEW_DEEP_DIVE.md` を参照するが、UI/BFF/FastAPI/SSE/課金/セキュリティ境界の正本は本ファイルに置く。
...
プロンプト本文は `InstructionId` 付きの `PromptInstruction` として `PromptPlan` に集約し、同じ意味の指示は ID 単位で重複排除してから `PromptRenderer` が描画する。これにより、既存の制約そのものは維持しつつ、文体・結論ファースト・構成・文字数・企業利用方針の重複出力を抑える。
...
`logic_patterns.py` は `es_reference_guidance.py` に入った抽象化済み構成ヒントの整形だけを担う。
...
- **指示の一元化**: 結論ファースト、だ/である調、事実追加禁止、企業固有情報の指示は `InstructionId` と policy フィールドで管理し、同じ意味の指示を PromptPlan で重複排除する
