...
| ガクチカ要点整理（構造化サマリー） | ログイン必須 | 6 credits を予約 -> 成功確定 / 失敗取消。`source === "llm"` かつ保存成功時のみ確定。fallback / キャッシュ命中は非課金                       | `gakuchika_summary`  |
...
| 志望動機フィードバック整理（FB） | ログイン必須 | 6 credits を予約 -> 成功確定 / 失敗取消。キャッシュ命中 / LLM 失敗 / 空応答 / 保存失敗は非課金                                            | `motivation_summary` |
| 面接会話          | ログイン必須 | start 2 credits、回答/続き 1 credit を予約 -> 成功確定 / 失敗取消                                                           | `interview`          |
...
- 要点整理（面接で話す要点のサマリー）:
  - ガクチカ要点整理（構造化サマリー）: 6 credits（従来は無料）。TransactionType: `gakuchika_summary`
    - BFF `interview-summary/route.ts` が `source` を判定し、LLM 生成成功（`source === "llm"`）かつ保存成功時のみ confirm。fallback・キャッシュ命中・保存失敗・confirm 失敗は非課金
  - 志望動機フィードバック整理（FB）: 6 credits。TransactionType: `motivation_summary`
    - `POST /api/motivation/[companyId]/feedback-summary`。キャッシュ命中（同一 draft の保存済みサマリ）・LLM 失敗・空応答・保存失敗・confirm 失敗は非課金。予約は認可・準備不足判定・キャッシュ判定の通過後にのみ行う
  - どちらも `reserve -> confirm/cancel`、`FEEDBACK_SUMMARY_CREDIT_COST = 6`（`src/lib/credits/shared.ts`）
- レジュメ深掘り:
...
- 面接対策全体がログイン必須
- start: 2 credits（`reserve -> confirm/cancel`）
- stream: 回答送信 1 credit（`reserve -> confirm/cancel`）
- continue: 講評後の続き 1 credit（`reserve -> confirm/cancel`）
...
| ガクチカ要点整理     | `claude-sonnet` | `claude-sonnet-4-6`      | Claude Sonnet 4.6 |
| 志望動機会話       | `claude-haiku`  | `claude-haiku-4-5-20251001` | Claude Haiku 4.5 |
| 志望動機下書き      | `claude-sonnet` | `claude-sonnet-4-6`      | Claude Sonnet 4.6 |
| 志望動機要点整理（FB） | `claude-sonnet` | `claude-sonnet-4-6`      | Claude Sonnet 4.6 |
...
- 全会話系機能は成功時のみクレジットを消費する。面接は start 2 credits、回答/続き各 1 credit、最終講評 6 credits
...
| 面接開始（plan + opening）              | GPT-5.4 + Claude Haiku 4.5 | ~¥2.4 | 2       | ~¥1.2       |
| 面接会話（1ターン, sliding window）       | Claude Haiku 4.5  | ~¥1.2          | 1       | ~¥1.2       |
| 面接続き（講評後の追加深掘り）              | Claude Haiku 4.5  | ~¥1.2          | 1       | ~¥1.2       |
