...
| 企業 RAG の月次無料ページ | 0 | 10 | 100 | 300 |
| 1 社あたり RAG ソース上限 | 0 | 3 | 100 | 500 |
| PDF 取込上限（1 ファイル） | 0 | 24 ページ | 72 ページ | 120 ページ |
| OCR 上限（1 ファイル） | 0 | 10 ページ | 32 ページ | 48 ページ |
...
| 志望動機 ES 下書き | ログイン必須 | 6 credits を予約 -> 成功確定 / 失敗取消 | `motivation_draft` |
...
| 企業 RAG PDF 取込 | ログイン必須 | 月次無料ページを消費しつつ、請求は PDF tier 表で固定 | `company_fetch` |
...
PDF は URL と違い、無料枠の消費有無にかかわらず、請求自体はページ帯で固定する。
...
- OCR も別上限で先頭ページのみ実行する。
- route 応答では `freeUnitsApplied`, `creditsConsumed`, `actualCreditsDeducted`, `processingNoticeJa` を返す。
...
- `monthlyAllocation = 0`
- `nextResetAt = null`
- `monthlyFree.companyRagPages = 0/0`
...
| `company_info_monthly_usage` | `rag_ingest_units`, `schedule_fetch_free_uses` |
