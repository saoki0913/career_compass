...
- **対応モデル**: 会話生成は `MODEL_GAKUCHIKA=gpt-mini`、ES 下書き生成は `MODEL_GAKUCHIKA_DRAFT=claude-sonnet`、構造化サマリー（要点整理）は `MODEL_GAKUCHIKA_SUMMARY=claude-sonnet`
- **プロトコル**: SSE (Server-Sent Events) によるリアルタイム質問ストリーミング
- **認証**: ログインユーザー専用（ゲストは会話・ES 生成ともに利用不可）
- **課金**: 会話は 1 問あたり 1 クレジット（成功時のみ消費）、ES 下書き生成は 6 クレジット（Reserve -> Confirm/Cancel）、構造化サマリー（要点整理）は 6 クレジット（Reserve -> Confirm/Cancel、成功時のみ消費）
...
`interview_ready` 到達時に `STRUCTURED_SUMMARY_PROMPT` を使って構造化サマリーを生成する。生成は `feature="gakuchika_summary"` で `MODEL_GAKUCHIKA_SUMMARY=claude-sonnet` に解決する（従来は `feature="gakuchika"` のままで Haiku に解決していたが、テレメトリ不整合を是正し Sonnet に統一した）。
...
構造化サマリー（要点整理）は成功時のみ 6 クレジット消費する（従来は無料）。BFF `interview-summary/route.ts` が `source` を判定し、LLM 生成成功（`source === "llm"`）かつ DB 保存成功のときだけ confirm する。フォールバック（`source === "fallback"`、LLM 失敗時の代替生成）・キャッシュ命中（同一 session / draft の保存済みサマリ）・DB 保存失敗・confirm 失敗は非課金。予約は認可・準備不足判定・キャッシュ判定をすべて通過した後にのみ行う。
...
| 構造化サマリー（要点整理） | 6 | Reserve -> Confirm/Cancel | `reserveCredits(userId, 6, "gakuchika_summary")`。`source === "llm"` かつ保存成功時のみ Confirm。fallback / キャッシュ命中 / 保存失敗 / confirm 失敗は非課金 |
...
### 構造化サマリー（要点整理）クレジット処理フロー

```
BFF interview-summary/route.ts
  |
  +-- 認可・準備不足判定・キャッシュ判定（命中時はここで return、予約なし＝非課金）
  +-- reserveCredits(userId, 6, "gakuchika_summary")  -> 6 クレジット事前控除
  +-- generateGakuchikaSummaryWithTelemetry()         -> source: "llm" | "fallback"
  +-- source !== "llm"            -> cancelReservation()（fallback は非課金）
  +-- DB 保存（contents.summary / conversations）
  +-- 保存失敗                    -> cancelReservation()（非課金）
  +-- 保存成功 -> confirmReservation()  -> 控除確定（confirm 失敗時は cancel して非課金）
```
