...
ES添削時に、ユーザーが選択して保存した企業ソースだけを使う RAG（Retrieval-Augmented Generation）システム。添削セッション中に URL を自動取得してコーパスを補強したりインデックスを更新したりはしない。
...
|------|------|----------|-----|
| 月次無料枠（ページ・URL+PDF 合算） | 10 | 100 | 300 |
| 1社あたり保存上限 | 3 source | 100 source | 500 source |
| PDF 1ファイルあたり取込ページ上限（超過は先頭から切り詰め） | 24 | 72 | 120 |
| PDF OCR 最大ページ（画像系で OCR 時、先頭から） | 10 | 32 | 48 |
