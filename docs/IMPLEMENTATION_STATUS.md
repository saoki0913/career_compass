...
| 部分成功（0.5クレジット） | 🔴 未実装 | 締切抽出失敗時の部分消費 |
...
| フェーズ管理 | 🟡 部分実装 | JSON フィールドあり、テンプレ未実装 |
| 職種管理 | ✅ 完了 | `jobTypes` テーブル |
| 応募枠上限（10枠/企業） | 🔴 未実装 | バリデーション未追加 |
| フェーズテンプレ | 🔴 未実装 | インターン/本選考テンプレ |
...
| 未読/既読管理 | ✅ 完了 | |
| すべて既読ボタン | ✅ 完了 | |
| 通知タイプ別ON/OFF | 🟡 部分実装 | スキーマあり、UI未実装 |
...
| テンプレ作成上限 | 🔴 未実装 | Free: 3, Standard: 10等 |
...
3. ✅ 日次通知バッチ処理 → **実装完了**
4. 🔴 採用ページ候補3件提示（企業情報取得のUX改善）

### 中優先度
5. ✅ 提出済み連動（タスク一括完了） → **既に実装済み**
6. 🔴 フェーズテンプレート
...
**実装完了率: 約 73%**（部分実装を50%として計算すると約 81%）
