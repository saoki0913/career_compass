...
- unit ルール:
  - URL: 1ページ = 1 unit
  - PDF: 10pまで 2 / 30pまで 4 / 60pまで 6 / 61p以上 10
- 未取得時:
  - 企業接続評価を出さない
...
### 4.3 ES添削
- 見積:
  - 標準: 〜800字 10 / 〜1600字 12 / 1601字〜 16
  - 低コスト: 〜800字 4 / 〜1600字 6 / 1601字〜 8
- 1回上限: 16
- 上限超過時:
  - 全文一括の実行はできない
...
- Googleカレンダー権限の同意は `/calendar/settings` の明示操作時のみ行う
...
### 14.2 Googleカレンダー設定
- 連携・再連携・解除はすべて `/calendar/settings` で行う
...
- 初回質問は `company_reason` から開始し、その後に `desired_work` を早い段階で聞く
...
- `company_reason / desired_work / fit_connection / differentiation / closing` を LLM が生成し、4択は LLM 出力 + grounded fallback で返す
- 中断/再開が可能
...
- `志望動機ESを作成` CTA は desktop では右カラム上部、mobile では入力欄直上に常時表示する
