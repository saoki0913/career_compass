...
- 初回案内は機能一覧モーダルではなく、ダッシュボード上の「最初にやること」導線で行う
...
### 4.3 ES添削
- 見積:
  - Claude: 〜500字 8 / 〜1000字 10 / 〜1500字 12 / 1501字〜 14
  - GPT: 〜500字 6 / 〜1000字 8 / 〜1500字 10 / 1501字〜 12
  - Gemini: 〜500字 6 / 〜1000字 8 / 〜1500字 10 / 1501字〜 12
  - 低コスト: 〜500字 2 / 〜1000字 3 / 〜1500字 4 / 1501字〜 5
- 1回上限: 14
- 上限超過時:
  - 全文一括の実行はできない
...
### 6.3 完了後の遷移
- ダッシュボードへ遷移し、「最初にやること」導線または今日のおすすめタスクを表示
...
- 同じ素材に対して再実行できる
  ...
  - 完了後は `深掘りを続ける` で同じセッションを再開できる
- クレジット消費は5問回答ごとに1（5問未満で終了した場合は消費なし）
- 次質問生成のAI出力は `star_scores` / `question` / `target_element` の最小構成を前提とする
...
- 素材は汎用として登録する
- 企業紐づけ用の DB カラムは残っているが、現行の深掘り画面と深掘り API では使用しない
- 素材は編集/削除できる
- ガクチカ素材は並び替え可能
...
- 回答候補は `2〜4件` の `suggestions / suggestionOptions` を返し、直接回答文だけを表示する
