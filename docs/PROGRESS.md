...
| テンプレ作成上限 | ✅ 完了 | Free: 3, Standard: 10, Pro: 15, Guest: 1 |
| テンプレ共有リンク | 🟡 部分実装 | スキーマあり、有効期限30日未実装 |
| 公開ギャラリー | ✅ 完了 | `/templates/gallery` |
| いいね/お気に入り | ✅ 完了 | |
| ランキング（週/月/年） | ✅ 完了 | ギャラリー内でソート可能 |
| 検索・フィルタ | ✅ 完了 | |
| スパム対策（投稿制限5件/日）（16.9） | 🔴 未実装 | 公開ギャラリー投稿制限 |
...
| 機能 | 状態 | 備考 |
|------|------|------|
| ES添削レートリミット | 🔴 未実装 | 10回/時間 |
| 企業情報取得レートリミット | 🔴 未実装 | 5回/分 |
| 異常パターン検知・ブロック | 🔴 未実装 | |
...
**実装完了率: 約 70%**（部分実装を50%として計算すると約 78%）

※ 検索機能が実装済みであることを確認し、統計を更新（2026-01-30）
