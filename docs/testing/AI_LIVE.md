...
- `localhost / make ai-live-local`: ローカル開発中に 7機能を一括実行する（**現在の正本**）
...
- **ES添削**: `make ai-live-local` の一括実行では `pytest` のみ実行し、ブラウザ E2E（`live-ai-major.spec.ts`）はスキップする（`AI_LIVE_SKIP_ES_REVIEW_PLAYWRIGHT=1` を wrapper が付与）。commit 前の feature 単体検証では `make test-e2e-functional-local-es` を使い、browser E2E まで必須で通す。
...
### 4-2. localhost / 7機能一括実行

ローカル開発中に `企業情報検索` `選考スケジュール取得` `企業RAG取り込み` `ガクチカ作成` `志望動機作成` `面接対策` `ES添削` の 7機能をまとめて回すときは、`make ai-live-local` を使う。実行順は `企業情報検索 → 選考スケジュール → RAG → 会話3機能 → ES添削`。

**この入口では ES 添削の Playwright（ログイン〜ストリーム）は走らない。** 添削の実 API 検証は同バンドル内の `es-review-pytest` のみ。browser を含めた localhost 破損検知は `make test-e2e-functional-local-es` を使う。
...
- stable status manifest: `backend/tests/output/local_ai_live/status/<feature>.json`
- feature ごとの正本 bundle（`make ai-live-local` の実行順に合わせた列挙）:
  - `live_company_info_search_*.json`, `live_company_info_search_*.md`
...
wrapper は必要なときだけ `npm run dev` と FastAPI（`tools/start-fastapi-playwright.sh` 相当の `uvicorn`）を起動し、既に応答する Next / FastAPI があればそのまま再利用する。`scripts/ci/check-ai-live-auth.mjs` に加えて `scripts/ci/check-ai-live-principal.mjs` を実際に採用した `--base-url` で 1 回だけ通し、BFF → FastAPI principal 経路が localhost で成立していることを fail-fast で確認する。これにより `CAREER_PRINCIPAL_HMAC_SECRET` の欠落や不整合を feature 実行前に止める。stateful 4機能の直前には `scripts/ci/reset-ai-live-state.mjs` を実行する。途中で feature が失敗しても残りは継続し、最後に summary を必ず生成する。
...
### 4-3. commit 前の localhost E2E 強制

- canonical enforcement は repo-managed `.githooks/pre-commit`
- 対象は staged files から判定した AI feature のみ
- 変更 feature ごとに `backend/tests/output/local_ai_live/status/<feature>.json` を確認し、`snapshotHash` が現在の staged snapshot と一致し、`status=passed` であることを要求する
- `es-review` は追加で `playwrightStatus=passed` を要求する
- `post-edit-dispatcher.sh` は block せず、対応する `make test-e2e-functional-local-*` を 1 セッション 1 回だけ案内する

### 4-4. 会話3機能の extended ケースとモデル行列（ローカル）
...
`scripts/ci/run-ai-live.sh`, `scripts/ci/run-e2e-functional.sh`, `scripts/ci/run-main-e2e.sh` は、shell に secret が入っていない場合でも `career_compass/github-actions.env` から `CI_E2E_AUTH_SECRET` と LLM keys を自動読込する。ローカルから `make test-e2e-regression` / `make test-e2e-functional-*` / `make test-e2e-functional-local-*` を叩くときは、この bundle が最新であることを前提にする。
