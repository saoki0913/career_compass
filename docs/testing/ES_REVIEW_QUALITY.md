...
- safe rewrite / degraded 採用が常用経路になっていないかを監視する
...
- `focused_retry_for_small_length_miss`
  - deterministic repair で救えない small miss を文字数特化 focused retry で回収できるか
- `gpt_length_focus_retry`
  - 390〜400 字設問で、strict → focused retry 1 → focused retry 2 の範囲で収束できるか
- `multi_focus_retry`
  - `under_min + verbose_opening` や `under_min + grounding` のような複合失敗で、`length` を主因、`opening / grounding` を副因として同じ retry に載せて収束できるか
- `gpt_focused_length_window`
  - 文字数不足または小超過が文字数特化 focused retry の target window で回収されるか
...
- `safe_rewrite`
  - 非 length 主因の複合失敗で safe rewrite が発火し、`safe_rewrite_triggered`、`safe_rewrite_reason`、`final_acceptance_source=safe_rewrite` が残るか
...
- `under_min` を含む mixed failure では、retry code と target window の主因が `under_min` 側に倒れること
- `self_pr` の自己否定語 (`経験不足` / `自信がない`) が `negative_self_eval` として検知され、`positive_reframe_focus` に乗ること
- 文末未完了の断片が `fragment` として弾かれ、文字数 focused retry の候補に紛れないこと
- rewrite validation retry
- retry ごとの prompt 圧縮
  - focused retry 2 回までで止まること
  - focused retry が低温化されても短字数帯・長字数帯で通ること
...
- `review_meta` で `classification_confidence` / `classification_secondary_candidates` / `recommended_grounding_level` / `effective_grounding_level` / `safe_rewrite_triggered` / `safe_rewrite_reason` / `final_acceptance_source` / `misclassification_recovery_applied` を確認できること
...
### 3-bis. Phase 10 散文品質テスト

Phase 10（冒頭 20-45 字 / 企業名言及回数 / 固有名詞汎用語置換 / self_pr 数値含有 / gakuchika ナンバリング）は validation を追加せずプロンプト層で誘導する設計のため、`backend/tests/es_review/test_es_review_prompt_structure.py` と `backend/tests/es_review/test_reference_es_quality.py` に**プロンプト文字列レベルの回帰テスト**を追加している。

`test_es_review_prompt_structure.py` の主な Phase 10 ケース:

- `test_constraints_require_opening_conclusion_with_20_to_45_chars` — rewrite / fallback の `<constraints>` に 20〜45 字ルールが含まれる
- `test_assistive_grounding_limits_company_name_mentions` — assistive 分岐の `company_mention_rule` が「本文全体で 2 回まで」を含む
- `test_required_grounding_limits_company_name_to_once_then_honorific` — required 分岐は「1 回まで」を含む
- `test_intern_reason_prompt_includes_proper_noun_policy` / `test_gakuchika_prompt_does_not_include_proper_noun_policy` — `_format_proper_noun_policy` のテンプレ別発動
- `test_template_specific_quantify_rules_are_only_in_self_pr_and_work_values` — 数値化ルールが self_pr / work_values にのみ適用される
- `test_basic_prompt_does_not_include_quantify_rule` — 他テンプレに漏れない
- `test_gakuchika_prompt_includes_structure_rule_and_playbook` — gakuchika のナンバリングルールと playbook 拡張
- `test_retry_guidance_has_quantify_and_structure_entries` — `TEMPLATE_DEFS` の retry_guidance に Phase 11 前提のキー追加

`test_reference_es_quality.py` の主な Phase 10 ケース:

- `test_quality_hints_require_short_opening_conclusion` — 9 テンプレ全ての 1 番目のヒントに「20〜45 字」
- `test_company_motivation_quality_hints_warn_on_repeating_company_name` — 企業名 3 回以上の NG
- `test_intern_and_role_quality_hints_include_proper_noun_generalization` — `本インターンシップ` / `本プログラム` / `本コース` への言い換え誘導
- `test_self_pr_and_work_values_quality_hints_require_numbers_and_actions` — 数値と具体的な行動動詞の要求
- `test_gakuchika_quality_hints_warn_against_listing_without_order` — `また / さらに` の羅列禁止

**Phase 11 で実装済み:** `test_reference_es_quality.py` の `structural_patterns_v2` 系 7 件（`test_build_reference_quality_profile_*` 系）は Phase 11 で製品仕様として実装された。`capture_kind` フィルタリング、結論先行 + 具体性ヒント条件分岐、構成パターン抽出（star_sequential / numbered_reasons / single_thread）を含む。全 27 件パス。

**Phase 12-15 追加改善:**
- SSEエラー返却方針の整理（`_extract_user_facing_message` による内部情報非露出）
- God Object 分割（`es_review_orchestrator.py` に4関数抽出、`es_review.py` を46行オーケストレータ化）
- AI臭検出強化（`concrete_value_absence`, `ai_added_kankeisha`, 「多角的に」「包括的に」追加）
- 具体性ルール修正（「数値を最低1つ入れる」→「行動の対象・範囲・頻度・比較を具体化。元回答にない数字は作らない」）
- review_meta 拡張（`ai_smell_tier`, `concrete_marker_count`, `opening_conclusion_chars`, `rewrite_sentence_count`）
- 文レベル diff 可視化（`text-diff.ts` + ReflectModal UI改善）
- 3軸品質スコア表示（論理性 / 具体性 / 企業適合）
- 構成パターンの構成ガイド追加（STAR順・番号理由・一貫論理の具体的な字数配分指示）

### 3-ter. エラーパステスト

`backend/tests/es_review/` のユニットテストは、正常系だけでなく以下のエラーパスもカバーする。

- **LLM 障害** — `rate_limit`、汎用エラー、`data=None` のいずれでも `HTTPException(503)` を返すこと
- **Total rewrite 失敗** — `REWRITE_MAX_ATTEMPTS` 回のリトライ後も文字数・品質条件を満たさない場合に `HTTPException(422)` を返すこと
- **SSE エラーイベント変換** — `503` などの HTTP エラーが SSE `error` イベントに正しく変換されること
- **複合失敗リトライパターン** — `under_min + style`、`grounding + over_max` のような複合失敗で、主因・副因を正しく識別し focused retry に載せられること

### 3-quater. テスト境界値定数

品質ゲートの判定に使う主要閾値。変更時はテストの期待値も連動して更新する。

| 定数 | 値 | 定義箇所 |
|---|---|---|
| `FINAL_SOFT_MIN_FLOOR_RATIO` | `0.9` | `backend/app/services/es_review/validation.py` |
| `REWRITE_MAX_ATTEMPTS` | `3` | `backend/app/services/es_review/retry.py` |
| `SSE_KEEPALIVE_INTERVAL_SECONDS` | `15.0` | `backend/app/routers/es_review.py` |
| AI smell tier 2 threshold (gakuchika short) | `4.0` | `backend/app/services/es_review/ai_smell.py` |
| AI smell tier 2 threshold (gakuchika mid_long) | `5.0` | `backend/app/services/es_review/ai_smell.py` |
| AI smell tier 2 threshold (_default short) | `4.5` | `backend/app/services/es_review/ai_smell.py` |
| AI smell tier 2 threshold (_default mid_long) | `5.5` | `backend/app/services/es_review/ai_smell.py` |

### 3-quinquies. Mock インフラストラクチャ

`backend/tests/es_review/conftest.py` に共通テストダブルを集約し、ES添削テストスイート全体で再利用する。

- **`FakeLLMResult`** — LLM 呼び出し結果の基底テストダブル。`LLMResultLike` Protocol（`backend/app/utils/llm_providers.py`）に準拠する構造的型付け
- **Factory メソッド** — `text_ok`, `text_fail`, `json_ok`, `json_fail`, `rate_limit_error` の 5 種。テストケースで頻出する成功・失敗パターンを 1 行で生成
- **後方互換サブクラス** — `FakeJsonResult`, `FakeTextResult` は既存テストの呼び出しパターンを壊さずに `FakeLLMResult` を継承
- **`LLMResultLike` Protocol** — `backend/app/utils/llm_providers.py` で `@runtime_checkable` として定義。`success`, `data`, `error`, `usage` の 4 属性を持つ構造的型。本番 `LLMResult` とテストダブルの両方がこの Protocol を満たすことを `conftest.py` 末尾の `assert isinstance(...)` で静的に保証

### 3-sexies. 入力データ多様性（extended ケース拡張）

extended ケースセットは、実際の就活生が書く ES の多様性を再現するため、以下の軸で入力バリエーションを拡張している。

**学生バックグラウンド:**
- 接客アルバイト経験（`company_motivation_zero_rag_unknown_company_short`）
- サッカー部キャプテン（`intern_reason_club_activity_background_medium`）
- 留学経験（`gakuchika_extreme_short_100char`）
- NPO / ボランティア活動（`post_join_goals_extreme_long_2000char`）
- 家庭教師（`self_pr_part_time_numeric_retention_medium`, `work_values_role_grounded_short`）

**企業パターン:**
- RAG 0% の未知企業（`company_motivation_zero_rag_unknown_company_short` — 企業名「ベクトル」、`rag_sources` 空）

**極端な文字数:**
- 最小 100 字（`gakuchika_extreme_short_100char` — `char_max=100`）
- 最大 2000 字（`post_join_goals_extreme_long_2000char` — `char_max=2000`）

**テンプレート網羅:**
- `company_motivation`, `gakuchika`, `post_join_goals`, `intern_reason`, `self_pr`, `work_values`, `role_course_reason`, `intern_goals`, `basic` の 9 テンプレートをカバー
...
- 現在の `extended` は **36 ケース**（smoke 8 + extended-only 28）。主要テンプレ × 文字数帯 × grounding に加え、次の hard-input を含む。
  - 箇条書き・メモ書きからの再構成
...
- **決定論**: `evaluate_live_case`（文字数、`だ・である`、`rewrite_validation_status`、メタの provider/model、`grounding_mode`、証拠数、必須トークン、422 相当の `HTTPException` など）。
...
| `LIVE_ES_REVIEW_CAPTURE_DEBUG` | `1` のときのみ、ルータが rewrite / focused retry の**各試行の生成テキスト**と却下理由を `ReviewMeta` の exclude フィールド（`rewrite_attempt_trace` / `rewrite_rejection_reasons`）に保持する。422 時は `detail.debug` に `rewrite_attempt_trace` も付与。統合テスト `test_live_es_review_provider_report` は実行時に `1` を強制する。 |
...
- `safe_rewrite_triggered` / `safe_rewrite_reason`: safe rewrite の発火監視
- `final_acceptance_source`: `rewrite` / `safe_rewrite` / `degraded_best_effort` の最終採用経路監視
- `misclassification_recovery_applied`: 近接テンプレ混線を抑える補助ガードの有無

`rewrite_validation_status` は `strict_ok` / `soft_ok` / `degraded` の判定に使う。runtime / gate ともに、通常の提出文は strict 受理帯を守る。文字数不足は専用パスではなく focused retry の target window と delta band guidance で回収する。`rewrite_validation_codes` は degraded 時も単一主因ではなく、最終未解決 code 群を保持する。`dearu_style` は末尾の空白を `strip()` してから **だ・である調** 判定し、style fail は最終検証前に安全な決定論正規化を一度だけ試す。
...
- ライブゲートの文字数は原則 `[char_min, char_max]`。`review_meta.length_policy == soft_ok` の場合だけ最終段の soft を許容する。許容帯は全帯域で **`0.9X〜X`**
