...
### 4. Next.js pre-stream policy tests

`src/app/api/documents/[id]/review/stream/route.test.ts` は、添削直前の企業情報補強が無駄打ちしないことを確認する。

- trusted official / parent-allowed source が揃っているときは起動しない
- third-party source しかないときは起動する
- assistive 設問で企業シグナルがないときは起動しない
