...
| 2 | 本番 Backend Health | `https://shupass-backend-production.up.railway.app/health` | Uptime / HTTP | 200 |
| 3 | 本番 Backend Ready | `https://shupass-backend-production.up.railway.app/health/ready` | Uptime / HTTP | 200 |
...
| Sentry projects | `sentry projects list` / `sentry project view ...` | `career-compass-frontend` / `career-compass-backend` は active。frontend は First Event あり、backend は No events yet。project view の Monitors は No のため、必須 3 monitors は dashboard 手作業 |
...
1. Sentry Dashboard で必須 3 monitors を作成し、email alert と green 状態を確認する。
