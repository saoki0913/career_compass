...
- secrets inventory sync: `make ops-secrets-sync` または `scripts/release/sync-career-compass-secrets.sh --check|--apply`（正本は codex-company `.secrets/career_compass`。`codex-company/.secrets/` 内の実ファイルを直接読まず、一覧・検証はこのスクリプト経由に寄せる）。`--check` は key 名の存在差分だけを見て、値は出さない。Claude Code 側の `codex-company/.secrets/` 直接 Read / cat / grep 等は `.claude/hooks/secrets-guard.sh` が PreToolUse で block する（詳細は [`docs/ops/AI_HARNESS.md`](./AI_HARNESS.md) 5.3 節）。
