...
`.secrets/` バンドルを正本として、各プロバイダ（Vercel / Railway / GitHub / Supabase）の環境変数と同期する。
...
.secrets/ (gitignored, 正本)
  ...
  │     ├── supabase.env    → Supabase production project (`career-compass-db`)
  │     └── shared.env      → 共通変数
  ├── staging/
  │     ├── nextjs.env      → Vercel staging
  │     ├── fastapi.env     → Railway staging
  │     ├── supabase.env    → Supabase staging project (`career-compass-staging`)
  │     └── shared.env      → 共通変数
  ├── ci/
...
**解決順序**: `--secret-dir` / `CAREER_COMPASS_SECRETS_DIR` > repo local `.secrets/` > legacy fallback。
...
スクリプトを直接使う場合（実値は読まず key set のみ確認）:
...
SYNC_MODE=--apply TARGET=supabase-staging make ops-secrets-sync
...
SYNC_MODE=--apply TARGET=supabase-production make ops-secrets-sync
```
...
# Supabase 用: .secrets/production/supabase.env / .secrets/staging/supabase.env
