...
**ポリシー**: staging と production は別 Supabase project のため、DB rollback は対象環境ごとに判断する。ただしデータ破壊や互換性崩壊を避けるため、production では roll-forward を最優先とする。
...
> **WHY**: バックエンドが DB スキーマとの互換性を先に回復しないと、フロントエンドのロールバックも正常に動作しない可能性がある。
