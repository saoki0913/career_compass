...
- Claude Code での `codex-company/.secrets/` 配下への直接 Read / cat / grep などは `.claude/hooks/secrets-guard.sh` が PreToolUse で block します。インベントリ確認は `zsh scripts/release/sync-career-compass-secrets.sh --check` のみ許可です。詳細は [`docs/operations/development/AI_HARNESS.md`](../development/AI_HARNESS.md) 5.3 節を参照してください。
...
- 適用範囲:
  ...
  - scope `"ai-stream"`: ES review / motivation / gakuchika / interview の SSE ハンドラ。
