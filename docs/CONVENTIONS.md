...
| `release/` | 本番初期構築（provider 初期設定） | 本番環境を初回構築する人 | 本番初回構築の正本 |
...
- **`release/`** = 初回本番構築。Vercel / Railway / Supabase / Stripe / Google Cloud / Sentry / domain など provider 初期設定だけを書く。日常運用の正本は `operations/production/`、環境変数の正本は `operations/platform/ENVIRONMENT_VARIABLES.md`。

旧 `ops` ディレクトリ、旧 `release/setup/` サブディレクトリ、旧 `release/PRODUCTION.md`、旧 `release/setup/ENV_REFERENCE.md` は使いません。新規文書・リンク・計画の出力先に旧パスを書かず、上記 `operations/` 配下または `release/` 直下のどれかへ分類してください。
...
2. **本番初期構築か？**（provider dashboard 初期設定、domain、Stripe 初期構築） → `release/`
...
- **SSOT を複製しない**: 環境変数は `operations/platform/ENVIRONMENT_VARIABLES.md`、計画タスク状態は `plan/plan-tasks.json` が唯一の正本です。
