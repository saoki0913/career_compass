...
就活Pass では、まず `販売事業者・運営責任者・所在地・電話番号` の4項目を同じ請求時開示文言で運用できます。ただし、この案は消費者庁の考え方には寄せられる一方、Stripe の `Head of Operations` 要件では差し戻される可能性があります。差し戻しがあった場合は、`運営責任者` のみ実名表示へ切り替える前提で扱ってください。
