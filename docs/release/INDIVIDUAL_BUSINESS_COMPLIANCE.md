...
自宅住所や私用連絡先を公開面から切り離しつつ、Stripe 審査に対応するなら、次の順で進めるのが無難です。

1. 個人事業主として開業する
2. 事業用メールを用意する
3. 請求時に所在地・電話番号を遅滞なく開示できる運用を用意する
4. 必要な場合のみ事業用住所・事業用電話を用意する
5. 特商法ページに販売事業者名・運営責任者名・`support@shupass.jp` を直接記載する
6. Stripe 本番公開準備へ進む
...
- Stripe も日本向けの `Commerce Disclosure` で、個人事業者は住所・電話番号の requested 時の開示方式を案内しています。
- Stripe 審査では `運営責任者` の氏名公開を個別に求められたため、販売事業者名と運営責任者名は同じ個人名で公開します。
...
- 運営責任者は `青木 駿介`
- 所在地は公開せず、請求があった場合に開示する
- 電話番号は公開せず、請求があった場合に開示する
- 特商法ページの文言は環境変数ではなくページ本文に直接記載する
- Stripe 本番申請は、少なくとも特商法ページ更新と請求時開示運用が整ってから進める

補足:

- Stripe への本人確認そのものでは生年月日・自宅住所の提出が必要になる可能性がありますが、これは Stripe に対する非公開提出であり、ユーザー向け公開とは分けて考えます。
- Stripe の条件確認により `運営責任者` の氏名公開が必要になったため、就活Pass では `氏名は公開し、所在地と電話番号は請求時開示` という方針を採用します。
- 差し戻しが出た場合は、まず `電話番号の公開要否`、次に `所在地の公開要否` を見直します。
...
就活Pass では、この分布に加えて Stripe から `運営責任者` の氏名公開を求められたことを踏まえ、`販売事業者と運営責任者は個人名を公開し、所在地と電話番号は請求時開示にする` 方針を採用します。

この方針は `1` に近く、Stripe の審査要件に対応しつつ、自宅住所や私用電話番号の公開を避けるための折衷案です。
...
就活Pass には既に特商法ページがありますが、今後は販売事業者名と運営責任者名を個人名で固定し、ページ本文に直接記載する方針にします。環境変数で販売事業者名・所在地・運営責任者文言を切り替える前提にはしません。
...
- `販売事業者`: `青木 駿介`
- `運営責任者`: `青木 駿介`
- `所在地`: `請求があった場合、購入申込み前に遅滞なく電子メールにて開示いたします。開示をご希望の方は support@shupass.jp までご連絡ください。`
- `電話番号`: `請求があった場合、購入申込み前に遅滞なく電子メールにて開示いたします。開示をご希望の方は support@shupass.jp までご連絡ください。`
- `メールアドレス`: `support@shupass.jp`
- `販売 URL`: `https://www.shupass.jp`
- `特商法ページ URL`: `https://www.shupass.jp/legal`

個人事業者の所在地・電話番号については、消費者庁 Q17 の条件を満たすなら、次のような案内に差し替える運用が可能です。

`請求があった場合、購入申込み前に遅滞なく電子メールにて開示いたします。開示をご希望の方は support@shupass.jp までご連絡ください。`

所在地については、就活Pass では現時点で請求時開示とし、Stripe から追加で公開を求められた場合にバーチャルオフィス住所の利用を再検討します。
...
1. **Settings → Public details** (事業者情報・公開情報)
   - `Business name`: `青木 駿介`（必要に応じて屋号 `Harbor Works` を併記）
   ...
   - `Support address`: Stripe Dashboard の要件に従って設定する。公開ページでは所在地を請求時開示とし、追加で公開を求められた場合のみバーチャルオフィス住所を再検討する
...
4. **Products → 就活Pass Standard / 就活Pass Pro**
   - 各 Product の `Description` に [managed-config.json](../../src/lib/stripe/managed-config.json) と同じ自動更新・解約・返金方針文言が反映されているかを確認
...
1. `青木 駿介` と `support@shupass.jp` を前提に公開方針を docs に固定する（完了）
2. 所在地と電話番号を請求時に遅滞なく開示できる運用を用意する
3. `/legal` に販売事業者名・運営責任者名・請求時開示文言を直接記載する（完了）
...
- `青木 駿介` は販売事業者名・運営責任者名として固定します。
- 所在地と電話番号は、ユーザー向けには請求時開示で運用します。
...
- [ ] `青木 駿介` を販売事業者名・運営責任者名として公開する方針を決めた
- [ ] 所在地と電話番号は請求時開示にする方針を決めた
...
- [ ] `/legal` の所在地と電話番号が請求時開示になっていることを確認した
- [ ] 所在地と電話番号の開示請求に返信できる運用を確認した
...
就活Pass では、以下のような個人開発・個人事業主の実例も参考にしつつ、`氏名は公開し、所在地と電話番号は請求時開示` という方針を採用します。これらは主に、開業判断、屋号運用、住所公開対策、個人開発 SaaS の収益化文脈を補助的に理解するために参照します。
