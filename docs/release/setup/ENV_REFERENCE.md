...
→ [`docs/operations/platform/ENVIRONMENT_VARIABLES.md`](../../operations/platform/ENVIRONMENT_VARIABLES.md)

- 全変数リファレンス・環境別比較・設定場所・生成/取得方法・新規追加の判断フロー: 上記 SSOT を参照。
- 確認コマンド（`sync-career-compass-secrets.sh --check` 等）: SSOT の **E drift 不変条件 / Verification** を参照。
