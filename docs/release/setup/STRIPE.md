...
終了時に `STRIPE_PRICE_*` の追記用行が標準出力に出ます。Webhook 新規作成時の `STRIPE_WEBHOOK_SECRET` 値は標準出力に出さず、Stripe Dashboard で確認して `codex-company` の `vercel-production.env` バンドルへ反映してください。その後 `zsh scripts/release/sync-career-compass-secrets.sh --check --target vercel-production` → `zsh scripts/release/sync-career-compass-secrets.sh --apply --target vercel-production --vercel-env production` を実行します。
...
作成後、**Signing Secret** (`whsec_...`) を控える → `codex-company/.secrets/career_compass/vercel-production.env` の `STRIPE_WEBHOOK_SECRET`
