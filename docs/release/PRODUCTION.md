...
- [ ] Sentry の必須 3 monitors（本番 Frontend / Backend Health / Backend Ready）が green で、email alert が有効であることを確認
...
- Sentry projects: `career-compass-frontend` / `career-compass-backend` active。Replay は off。frontend は First Event あり、backend は No events yet。project view の Monitors は No のため、必須 3 monitors は dashboard 手作業。
