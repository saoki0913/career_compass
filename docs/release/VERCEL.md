...
> いまの標準運用では、Vercel の env は `scripts/bootstrap/career-compass/sync-career-compass-env.sh` で同期し、値の正本は codex-company 側の `.secrets/career_compass`（`vercel-*.env`）。一覧は [`docs/release/ENV_REFERENCE.md`](./ENV_REFERENCE.md) の Release Automation Inputs。release は `make deploy` から開始します。Dashboard の手動更新は初期セットアップやトラブルシュート時だけに寄せます。
...
> 本番用 Vercel project は `main` のみを production branch として扱います。`develop` は別の staging project に接続し、`https://stg.shupass.jp` へ出します。
...
| **Preview** | feature branch の一時プレビュー | 一時確認用。正式な OAuth / 書き込み確認には使わない |
...
> デフォルトドメイン (`career-compass-xxx.vercel.app`) はプレビュー環境として残しておくと便利です。
> ただし OAuth と書き込み系 API は `www.shupass.jp` と `stg.shupass.jp` のみを正式対応にします。
...
> 正式な staging は別 project の `stg.shupass.jp` です。`*.vercel.app` の preview URL は一時確認専用で、OAuth と書き込み系 API の保証対象にはしません。
