...
終了時に `STRIPE_PRICE_*` と（Webhook 新規時のみ）`STRIPE_WEBHOOK_SECRET` の追記用行が標準出力に出ます。`codex-company` の `vercel-production.env` バンドルへ反映し、`zsh scripts/release/sync-career-compass-secrets.sh --check --target vercel-production` → `zsh scripts/release/sync-career-compass-secrets.sh --apply --target vercel-production --vercel-env production` を実行してください。
