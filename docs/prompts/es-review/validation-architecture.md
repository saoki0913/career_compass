...
品質判定（結論ファースト・企業接地・文体統一・構造明瞭性・事実保持・表現重複・主題焦点）は LLM が担う。
...
| `expression_diversity` | 同じ概念を近接文でほぼ同じ表現のまま繰り返していないか | retry hint 付きで再試行 |
| `theme_focus` | 本文の主題が設問タイプに合っているか。ガクチカ詳細が他テンプレ本文を支配していないか | retry hint 付きで再試行。`gakuchika` では skip |
...
`ValidationProfile.axis_modes()` により、軸ごとに `required` / `warn` / `skip` が決まる。現行では短い元回答の `fact_preservation` は情報密度 tier に応じて warn に緩和され、`theme_focus` は `gakuchika` では skip する。
...
7. theme_focus: 本文の主題が設問タイプに合致しているか。設問が求める回答（動機・強み・ビジョン等）が本文の主軸になっているか。
</evaluation_criteria>
...
初回 rewrite / fallback / draft では詳細な `<anti_ai_phrase>` ブロックを出さず、`PromptInstruction` の compact advisory として代表的な定型句だけを抑止する。validation が AI 臭を検出した retry では、failure code に紐づく差分指示だけを `<retry>` に出す。カテゴリ定義は `ai_smell.py` を正本とし、検出ロジックと retry 指示が乖離しないようにする。
