...
WANT_COMMIT=false
if printf '%s' "$LOWER" | grep -qiE '(^|[^a-z0-9])commit([^a-z0-9]|$)|コミット'   && ! printf '%s' "$LOWER" | grep -qiE 'do not commit|don.?t commit|without committing|skip commit|no commit|コミットしない|コミットせず|コミット不要|commit[[:space:]]*しない'; then
  WANT_COMMIT=true
fi

WANT_PUSH=false
if printf '%s' "$LOWER" | grep -qiE '(^|[^a-z0-9])push([^a-z0-9]|$)|プッシュ'   && ! printf '%s' "$LOWER" | grep -qiE 'do not push|don.?t push|without pushing|skip push|no push|push[[:space:]]*はしない|push[[:space:]]*しないで|プッシュしない|プッシュせず|押さない'; then
  WANT_PUSH=true
fi
...
if [ -n "$SESSION_ID" ] \
  ...
  if printf '%s' "$PROMPT" | grep -qiE "$NEGATION_RE" && [ "$WANT_COMMIT" != "true" ]; then
    rm -f "$INTENT_FILE"
  else
    ...
    if [ -n "$TAXO" ]; then
      ...
      elif [ "$WANT_COMMIT" = "true" ] && [ "$WANT_PUSH" = "true" ]; then
        ACTIONS=$(printf '%s' "$TAXO" | jq -c '[.actions[] | select(. == "test" or . == "commit" or . == "push")]')
        RELEASE_MODES='[]'
      elif [ "$WANT_COMMIT" = "true" ]; then
        ACTIONS=$(printf '%s' "$TAXO" | jq -c '[.actions[] | select(. == "test" or . == "commit")]')
        RELEASE_MODES='[]'
      else
      ...
      if [ "$WANT_PROD" = "true" ]; then
        ...
        ACTIONS='["test","push","release","commit"]'
        RELEASE_MODES='["staging","release"]'
      elif [ "$WANT_COMMIT" = "true" ] && [ "$WANT_PUSH" = "true" ]; then
        ACTIONS='["test","push","commit"]'
        RELEASE_MODES='[]'
      elif [ "$WANT_COMMIT" = "true" ]; then
        ACTIONS='["test","commit"]'
        RELEASE_MODES='[]'
