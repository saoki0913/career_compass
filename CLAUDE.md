...
**code-reviewer はブロッキングレビュアー（blocking reviewer）** として扱う。plan_review / post_review では code-reviewer を 3 並列で起動し、厳しい方の判定を採用する。code-reviewer を起動した場合、3 件すべての結果が返るまでは実装判断の確定、計画の確定、commit / push、最終回答へ進まない。`/codex:status` や `wait_agent` が timeout しただけでは承認扱いにしない。明示的にユーザーが「code-reviewer を待たずに続行」と指示した場合のみ例外とし、その事実を最終回答に記録する。

**改善タスクの標準レビュータイミングは 2 回**。1 回目は Plan 案を作成した後、Plan を確定する前に実行し、計画の漏れ、リスク、確認方法を code-reviewer 3 並列で確認する。2 回目は修正完了後に実装差分を code-reviewer 3 並列で確認する。docs-only、test-only、局所的な文言修正、明らかな局所バグ修正では省略してよい。

**レビュー再実行ループは禁止**。同一の Plan 確定または commit 試行で code-reviewer を自動再実行できるのは、各タイミング 1 回までとする。指摘反映後は、再度 code-reviewer を走らせず、Claude が修正差分と対象テストで確認する。修正により新規 API、認証・課金・RLS・Stripe、DB スキーマ、release / deploy 周辺、または 300 行以上の追加変更が発生した場合のみ、AskUserQuestion でユーザーに確認してから追加レビューを実行してよい。
...
この規約は `.claude/hooks/stop-plaintext-confirm-guard.sh` が Stop hook で機械的に enforce する。transcript の最終 assistant メッセージに `commit/push/E2E/デプロイ/リリース/マージ/merge/PR/本番/削除/リセット/rebase` 系のトピック語（26種）と `しますか/ますか/お知らせください/必要であれば/Should I/Let me know` 系の確認・保留・提案語（34種）が両方含まれる場合、Stop をブロックし AskUserQuestion での再提示を要求する。
...
`src/components/**`, `src/app/**/(page|layout|loading).tsx`, `src/components/skeletons/**` を変更する際の推奨フロー:

1. 設計判断が必要な場面のみ事前: `npm run ui:preflight -- <route> --surface=marketing|product [--auth=none|guest]`
2. 変更後: `npm run lint:ui:guardrails`（marketing accent / loading skeleton 検査）
3. 変更後: `npm run test:ui:review -- <route>`（Playwright 5 viewport スモーク + 横スクロール検出）

参照: `docs/architecture/FRONTEND_UI_GUIDELINES.md`, `DESIGN.md`, `docs/marketing/LP.md`。PR の `UI Review Routes` (`.github/PULL_REQUEST_TEMPLATE.md`) は該当する場合に記入する。既存画面では既存のデザインシステムを優先する。
...
## Codex Integration (2+1 Channels)

Claude Code を司令塔（設計・リサーチ・検証・ユーザー対話）、Codex を実装エンジニアとして運用する。**Plugin + MCP** の 2 チャネルを主軸とし、**delegate.sh** を全モード fallback として維持する。

### アーキテクチャ

- **Claude Code（司令塔）**: 要件整理、壁打ち、設計、タスク分解、Codex への委譲判断、結果検証、最終統合
- **Plugin — codex-plugin-cc（レビュー・ジョブ管理）**: `/codex:review`, `/codex:adversarial-review`, `/codex:rescue`, `/codex:status`, `/codex:result`, `/codex:cancel`。fire-and-forget 委譲とバックグラウンドジョブ管理
- **MCP（対話的実装）**: `mcp__codex__codex` + `codex-reply` でマルチターンステアリング。重い実装で方向修正が必要な場面
- **delegate.sh（バッチ fallback）**: plan_review, implementation, post_review, imagegen の全 4 モード維持

### チャネル選択ガイド

| シナリオ | Primary | Fallback |
|---|---|---|
| コードレビュー（uncommitted） | Plugin `/codex:review` | delegate.sh post_review |
| 設計・アーキテクチャレビュー | Plugin `/codex:adversarial-review` | delegate.sh plan_review |
| 重い実装（ステアリング必要） | MCP `codex(heavy_impl)` + `codex-reply` | delegate.sh implementation |
| 軽量実装（fire-and-forget） | Plugin `/codex:rescue --background` | delegate.sh implementation |
| 並列サブエージェント調査 | MCP `codex(review_only)` × N | Plugin `/codex:rescue --background` × N |
| 並列サブエージェント実装 | Plugin `/codex:rescue --background` × N | delegate.sh implementation |
| 対話的デバッグ・修正指示 | MCP `codex(heavy_impl)` + `codex-reply` | — |
| 軽微修正（docs/test/文言） | MCP `codex(small_auto)` | — |
| 画像生成 | delegate.sh imagegen（exclusive） | — |
| ジョブ監視 | Plugin `/codex:status` | `ls -td .codex/state/handoffs/` |
| 結果取得 | Plugin `/codex:result` | Read result.md |

### Plugin コマンド（codex-plugin-cc）

| コマンド | 用途 |
|---|---|
| `/codex:review` | uncommitted 変更 or ブランチ比較のコードレビュー。`--background` / `--base <ref>` 対応 |
| `/codex:adversarial-review` | 設計判断・トレードオフ・仮定を問う深度レビュー（advisory、commit 非ブロッキング） |
| `/codex:rescue` | バグ調査・修正・タスク継続の委譲。`--background` で非同期実行可 |
| `/codex:status` | 実行中・最近のジョブ一覧 |
| `/codex:result` | 完了ジョブの結果取得 |
| `/codex:cancel` | アクティブなジョブの停止 |

### MCP ツール

- `mcp__codex__codex(prompt, cwd, model, profile, sandbox, approval-policy, ...)` — セッション開始
- `mcp__codex__codex-reply(prompt, threadId)` — 同一スレッド継続（最大 3 ターン、超過時は scope 見直し）

### MCP プロファイル

| 用途 | profile | sandbox | approval |
|---|---|---|---|
| 調査・レビュー・並列調査 | `review_only` | read-only | never |
| 通常実装・テスト・リファクタ | `heavy_impl` | workspace-write | on-request |
| docs/test/文言の軽微修正 | `small_auto` | workspace-write | never |

### MCP 対話的実装パターン

MCP は対話的ステアリングが必要な重い実装で使う。エージェント指定は対象ファイルパスを明示すれば `.codex/config.toml [routing]` が自動選択する。

```
1. [Claude] タスク分解・リッチコンテキスト準備
2. [Claude] mcp__codex__codex(prompt, cwd, profile=heavy_impl)
   - prompt に対象コード・パターン例・テスト期待値・制約を含む
   - エージェント指示: "fastapi-developer の指針に従って実装"
3. [Claude] 結果確認 → 追加指示は codex-reply（最大 3 ターン）
4. [Claude] git diff 検証 → tsc → テスト
5. 不合格 → codex-reply でフィードバック（1 回のみ）
6. まだ不合格 → Claude 直接修正
```

### cwd の指定

MCP `codex` ツールには必ず `cwd` を指定する。Codex がプロジェクトの `.codex/config.toml`, `AGENTS.md`, hooks を自動ロードする。

### 委譲判断ポリシー

通常の実装タスクは Claude Code の判断で Plugin or MCP に委譲してよい。

以下の操作は **AskUserQuestion でユーザーに確認してから** 委譲する:
- 認証・課金・RLS・Stripe の変更
- DB スキーマの破壊的変更
- release / deploy / provider CLI 操作
- package manager migration / major framework upgrade

確認時には委譲スコープ（変更対象ファイル一覧）、推奨チャネル、リスク評価を提示する。
...
1. Plan 案を作成した後、Plan を確定する前にレビュー対象、変更方針、確認方法をまとめる
2. Codex plan review は標準実行。実行前に **AskUserQuestion でユーザー確認が必須**（`codex-delegate-gate.sh` が機械的に enforce）:
   ...
   - 実行: プラン内容を `/tmp/codex-ctx-$(date +%s).md` に書き出し（`umask 077`、機密情報を含めない）、code-reviewer 3 並列で `bash scripts/codex/delegate.sh plan_review --context-file <path>` を実行する
...
3. plan review の code-reviewer 3 件が完了したら、指摘を Plan に反映して Plan を確定する。同じ Plan 確定前レビューは自動再実行しない
4. Plan 確定後、ExitPlanMode を呼ぶ（ブロックされない）
5. 最初の Edit|Write 実行時に `impl-start-codex-gate.sh` がブロック
6. AskUserQuestion で「この実装を Codex に委譲しますか？」と確認する。以下の情報を提示:
...
### B. 修正完了後コードレビュー自動化（post_review）

改善タスクでは、修正完了後かつ commit を作成する直前に post_review を標準実行する。docs-only、test-only、局所的な文言修正、明らかな局所バグ修正は省略してよい。省略対象か迷う場合は、以下の閾値チェックで判定する:
...
2. 改善タスク、または以下のいずれかに該当する場合、自動で Codex レビューを実行:
   - 変更ファイル数 >= 10
   - 変更行数（追加+削除）>= 500
   - hotspot ファイルの変更を含む（正本: `.claude/hooks/lib/skill-recommender.sh` の `HOTSPOT_FILES` 配列）
3. 該当時: **AskUserQuestion でユーザー確認後**、以下の primary / fallback でレビューを実行する:
   - **Primary**: `/codex:review`（codex-plugin-cc）。`--background` で非同期実行可
   - **Fallback**: `bash scripts/codex/delegate.sh post_review`（Plugin が使えない場合）。`codex-delegate-gate.sh` が AskUserQuestion 確認を enforce
4. レビュー結果の確認:
   - **Plugin 経由**: `/codex:status` でジョブ完了を確認 → `/codex:result` で結果取得
   - **delegate.sh 経由**: `ls -td .codex/state/handoffs/post_review-*/ .claude/state/codex-handoffs/post_review-*/ 2>/dev/null | head -1` → `meta.json` と `review.json` を Read
   - **code-reviewer 3 件の完了は必須**。他 agent のレビューが揃っていても、code-reviewer のいずれかが pending / running / timeout のままなら Section B-2、commit、push、最終回答に進まない。
5. Status 判定:
   - レビュー失敗 / TIMEOUT / CODEX_ERROR / PARSE_FAILURE → Claude 自身の code-reviewer skill で fallback review
   - APPROVE → commit を続行
   - REQUEST_CHANGES かつ maxSeverity == high → 指摘を修正し、修正差分と対象テストで確認してから commit。code-reviewer の自動再実行はしない
   - REQUEST_CHANGES で medium/low のみ → 指摘を commit メッセージに記録し commit 続行可
   - NEEDS_DISCUSSION → ユーザーに AskUserQuestion で相談
   - 追加レビューが必要なのは、修正により新規 API、認証・課金・RLS・Stripe、DB スキーマ、release / deploy 周辺、または 300 行以上の追加変更が発生した場合のみ。実行前に AskUserQuestion でユーザー確認を取る
6. **delegation 確認** — レビュー完了後、AskUserQuestion でユーザーに確認する。結果サマリ（status / 主要 findings）を含めること:
   - **commit 続行（Plugin 経由）** → `node scripts/harness/diff-snapshot.mjs checkpoint --kind commit-review --decision plugin-reviewed --project "$(pwd)" > ~/.claude/sessions/career_compass/codex-commit-delegation-$SESSION_ID`
   - **commit 続行（delegate.sh 経由）** → `node scripts/harness/diff-snapshot.mjs checkpoint --kind commit-review --decision reviewed-proceed --review-request-id "<review.json requestId>" --review-execution-status "<review.json executionStatus>" --review-verdict "<review.json reviewStatus>" --max-severity "<review.json maxSeverity>" --project "$(pwd)" > ~/.claude/sessions/career_compass/codex-commit-delegation-$SESSION_ID`
   ...
   - **Claude fallback** — レビューが TIMEOUT/CODEX_ERROR/PARSE_FAILURE の場合、Claude 自身の code-reviewer skill でレビュー後 `node scripts/harness/diff-snapshot.mjs checkpoint --kind codex-post-review --decision fallback-reviewed --project "$(pwd)" > ~/.claude/sessions/career_compass/codex-commit-delegation-$SESSION_ID`

   > **⚠** checkpoint は現在の staged diff と同じ状態に結び付けて作成すること。`commit-codex-gate.sh` は JSON checkpoint の `headSha` / `stagedDiffHash` を検証する。`plugin-reviewed` は review.json 照合をスキップし stagedDiffHash のみ検証する。`skip-review` は廃止済みで受理されない。
7. 省略対象の軽微変更かつ閾値未満の場合: Codex レビューをスキップし通常通り commit（`commit-codex-gate.sh` も素通り）

### B-3. Adversarial Design Review（設計深度レビュー）

post_review 通過後、以下の条件に該当する場合 `/codex:adversarial-review` を追加実行する。設計判断・トレードオフ・仮定の妥当性を問う深度レビュー。

**トリガー条件**（いずれか）:
- architect-routed ファイル（`.codex/config.toml [routing]`）の変更を含む
- 新規 API エンドポイントの追加
- auth/billing/security ファイルの変更
- post_review が REQUEST_CHANGES(medium) を返した場合

**性質**: advisory（commit をブロックしない）。重大な設計問題を発見した場合のみ AskUserQuestion でユーザーに相談する。

**Fallback**: `/codex:adversarial-review` が使えない場合、`scripts/codex/prompt-templates/adversarial-review.md` のテンプレートで delegate.sh plan_review を実行する。
...
pre-commit フック (`enforce-local-ai-e2e.mjs` + `scripts/security/run-lightweight-scan.sh`) がステージ済みファイルの `snapshotHash` / マニフェスト照合とセキュリティスキャンを行う。テスト実行でマニフェストが生成されるため、この順序でなければコミットがブロックされる。
...
4. `shouldRun: true` → AskUserQuestion (multiSelect: true) でテストカテゴリ選択を提示:
   ...
   → 選択されたカテゴリのテストを実行:
     ...
     - Security: `bash scripts/security/run-lightweight-scan.sh --staged-only --fail-on=critical`
5. テスト結果の処理:
   ...
   d. Security scan 失敗 (critical):
      → コミットをブロック。修正後に再スキャン
   e. 全 pass → Section B の修正完了後レビュー（Codex post_review）→ コミット → **ユーザーに push 確認**
...
- Codex `post_review`（Section B）は修正完了後に実行する。E2E が必要な変更では、E2E 通過後かつコミット直前に実行し、修正分もレビュー対象にする
...
- pre-commit hook は 2 段階: `enforce-local-ai-e2e.mjs` (E2E マニフェスト) + `scripts/security/run-lightweight-scan.sh` (セキュリティ)
...
**常に Claude が直接対応する（委譲禁止）:**
- 設計判断・アーキテクチャ変更
- プロンプトエンジニアリング（反復的評価が必要）
- secrets へのアクセスが必要なタスク
- ユーザーとの対話・方向性決定

**AskUserQuestion でユーザー確認後に委譲可能（MCP Integration セクションと統一）:**
- 認証・課金・RLS・Stripe の変更
- release / deploy / provider CLI 操作
- DB スキーマの破壊的変更
- package manager migration / major framework upgrade
...
2. [MCP] codex(heavy_impl) で対話的実装
3. [Claude] 結果確認 → 追加指示は codex-reply（最大 3 ターン）
4. [Claude] 検証 (tsc, unit test, diff review)
5. 不合格 → codex-reply でフィードバック (1回のみ)
6. まだ不合格 → [Claude] 直接修正
7. [Plugin] /codex:review (コードレビュー)
8. [Plugin] /codex:adversarial-review (§B-3 条件該当時)
9. [Claude] Section B-2 (stage → E2E → commit → push)
```

**並列委譲フロー（Plugin fire-and-forget）**:
```
1. [Claude] タスク分解 → 2-4 の独立サブタスク（ドメイン別）
2. [Claude] 各サブタスクにスコープ付きコンテキスト準備
3. [Plugin] /codex:rescue --background × N（各サブエージェント向け）
4. [Plugin] /codex:status で全ジョブ監視
5. [Plugin] /codex:result で全結果収集（code-reviewer を含む場合は 3 件とも必ず完了まで待つ）
6. [Claude] 結果統合・競合解決
7. [Claude] 検証 (tsc, unit test, diff review)
8. [Plugin] /codex:review (統合レビュー)
9. [Claude] Section B-2 (stage → E2E → commit → push)
```

**Fallback フロー（delegate.sh バッチ）**:
```
1. [Claude] リッチコンテキストファイル作成
2. [Codex] delegate.sh implementation (workspace-write)
3. [Claude] handoff 読み取り・検証
4. 不合格 → delegate.sh で再委譲 (1回のみ)
5. [Codex] delegate.sh post_review
6. [Claude] Section B-2
```

**並列安全性**: Codex 実行中は Claude はファイル編集を行わない。リサーチ・設計・ユーザー対話のみ実施する。
**レビュー待機安全性**: code-reviewer 実行中は「主要判断は揃ったので進める」「後から吸収する」などの推測で完了扱いにしない。結果が未返却なら待機状態として報告し、続行判断はユーザーの明示指示を待つ。

#### C-4. リッチコンテキスト準備

Claude-to-Codex 委譲では、Codex worker が外部リサーチやサブエージェントに依存しなくても完了できるように、Claude が委譲前にリサーチを完了し、結果をコンテキストファイルに含める。Codex ネイティブの開発ハーネスでは `.codex/agents` / skills / hooks を活用してよいが、Claude 起点の委譲タスクは self-contained にする。タスクの複雑度に応じて以下から選択:
...
チャネル選択ガイドに従い、タスクに最適なチャネルで委譲する:

**MCP（対話的実装 — primary）**:
1. リッチコンテキストをプロンプトに含めて `mcp__codex__codex(prompt, cwd, profile=heavy_impl)` を実行
2. 結果確認 → 追加指示は `codex-reply`（最大 3 ターン）
3. `git diff` で変更スコープを検証
4. 検証チェックリスト: `npx tsc --noEmit` → 関連テスト → スペック一致確認
5. 不合格 → `codex-reply` でフィードバック（1 回のみ）→ まだ不合格 → Claude 直接修正

**Plugin（fire-and-forget — 軽量タスク or 並列）**:
1. `/codex:rescue [--background]` でタスクを委譲（プロンプトにコンテキスト含む）
2. `/codex:status` で完了確認 → `/codex:result` で結果取得
3. 検証チェックリスト同上
4. 不合格 → MCP `codex-reply` or Claude 直接修正

**delegate.sh（バッチ fallback）**:
...
3. 最新の handoff: `ls -td .codex/state/handoffs/implementation-*/ | head -1`
...
- **BLOCKED / TIMEOUT / CODEX_ERROR / PARSE_FAILURE** → Claude が直接実装にフォールバック

#### C-6. フィードバック付き再委譲（1 回限り）

1. 問題箇所・エラーメッセージ・期待する修正をまとめる
2. MCP → `codex-reply` でフィードバック送信。delegate.sh → 新コンテキストファイルで再実行
3. 再委譲結果を同じ検証チェックリストで検証
4. それでも不合格 → Claude が直接修正する。再々委譲はしない

#### C-7. Codex レビュー（実装後）

Codex 実装完了後（Claude 直接実装の場合も Section B の閾値該当時）、レビューを実行する:
- **Primary**: `/codex:review`（Plugin）
- **Fallback**: `bash scripts/codex/delegate.sh post_review`
- code-reviewer agent 3 件がセキュリティ（OWASP Top 10）+ コード品質を並列検査
- code-reviewer 3 件の結果を取得するまで次工程へ進まない。pending / running / timeout は approve ではない。
- code-reviewer の指摘を修正した後は、同じ commit 試行では code-reviewer を自動再実行しない。修正差分の確認と対象テストで閉じる。
- §B-3 の条件に該当する場合 `/codex:adversarial-review` も追加実行
...
- ユーザーへの説明、進捗共有、最終報告は必ず日本語で書く。
- 英語の開発用語を日本語文の途中に不自然に混ぜない。
- 専門用語を使う場合は、初回だけ「日本語名（英語名）」の形で補足する。
- 2 回目以降は原則として日本語名を使う。
- ファイル名、関数名、クラス名、変数名、コマンド、ライブラリ名、API 名は原文のままにする。
- エラーメッセージ、ログ、差分表示、テスト名は原文を維持してよい。

### 計画表示のルール

計画を提示するときは、原則として以下の日本語見出しを使う。
...
計画や進捗説明では、以下のような英語表現をそのまま使わず、自然な日本語に置き換える。
...
| inspect | 確認する |
| implement | 実装する |
| refactor | 整理する / 責務を分ける |
| validate | 検証する |
| run tests | テストを実行する |
| update docs | ドキュメントを更新する |
| fix bug | 不具合を修正する |
| add fallback | 代替処理を追加する |
| handle error | エラー処理を追加する |
| check diff | 差分を確認する |
| apply patch | 修正を反映する |

### 禁止する表現

- "rate limit middleware を inspect して fallback behavior を validate します"
- "auth flow を trace して edge case を handle します"
- "tests を run して docs を update します"

### 推奨する表現

- "レート制限の処理を確認し、Redis が使えない場合の代替処理が正しく動くか検証します"
- "認証の流れを確認し、例外的なケースでも安全に処理できるようにします"
- "テストを実行し、ドキュメントを更新します"

### 最終報告の形式

最終報告は、原則として以下の順で簡潔に書く。
