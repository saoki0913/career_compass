...
Claude Code を司令塔（調査・設計・壁打ち）、Codex MCP を GPT サブスク枠の実装エンジニアとして運用する。delegate.sh（batch execution）と MCP（対話的実行）のデュアルチャネル構成。

### アーキテクチャ

- **Claude Code（司令塔・設計者）**: 要件整理、ユーザーとの壁打ち、アーキテクチャ設計、方針決定、タスク分解、実装範囲の絞り込み、Codex への委譲判断、git diff レビュー、最終統合
- **Codex MCP（実装エンジニア）**: 重い実装、リファクタリング、テスト追加、バグ修正、影響範囲調査、サブエージェント並列調査・レビュー、認証・課金・RLS・DB・deploy（ユーザー承認時）
- **delegate.sh = batch execution（従来通り維持）**: plan_review, implementation, post_review, imagegen

### Codex MCP ツール

- `mcp__codex__codex(prompt, cwd, model, profile, sandbox, approval-policy, ...)` — セッション開始
- `mcp__codex__codex-reply(prompt, threadId)` — 同一スレッド継続
...
通常の実装タスクは Claude Code の判断で Codex MCP に委譲してよい。

以下の操作は **AskUserQuestion でユーザーに確認してから** Codex MCP に委譲する:
- 認証・課金・RLS・Stripe の変更
- DB スキーマの破壊的変更
- release / deploy / provider CLI 操作
- package manager migration / major framework upgrade

確認時には委譲スコープ（変更対象ファイル一覧）、推奨プロファイル、リスク評価を提示する。

### delegate.sh との使い分け

| 場面 | 使うもの |
|---|---|
| 重い実装（対話的にステアリングしたい） | Codex MCP (`heavy_impl`) |
| バッチ実装（fire-and-forget でよい） | `delegate.sh implementation` |
| コードレビュー（構造化結果が必要） | `delegate.sh post_review` |
| 設計レビュー | `delegate.sh plan_review` |
| 画像生成 | `delegate.sh imagegen` |
| 調査・影響範囲分析 | Codex MCP (`review_only`) |
| サブエージェント並列調査 | Codex MCP (`review_only`) |
| docs/test/文言の軽微修正 | Codex MCP (`small_auto`) |

### 基本ワークフロー（3 段階）

1. **調査** — Claude Code が設計し、必要に応じて Codex MCP (`review_only`) でサブエージェント並列調査
2. **範囲決定** — Claude Code がユーザーと壁打ちして実装範囲を決定
3. **実装** — Codex MCP (`heavy_impl`) でエンジニアリング実装 → Claude Code が diff レビュー → 修正が必要なら `codex-reply` で追加指示

### サブエージェント並列調査の指示パターン

Codex MCP (`profile=review_only`) に以下のように指示すると、Codex 内部の routing と agents が並列実行する:

```
以下のサブエージェントを並列に使ってください:
1. architecture-reviewer — 責務分担と境界の調査
2. security-reviewer — 認証・RLS・入力検証のリスク調査
3. test-engineer — テスト不足の調査
最後に優先度順で統合してください。
```

### 重い実装の委譲パターン

Codex MCP (`profile=heavy_impl`) への実装委譲時のプロンプト構造:

```
cwd: /Users/saoki/work/career_compass
profile: heavy_impl
...
Codex MCP の `codex` ツールには必ず `cwd` を指定する。これにより Codex がプロジェクトの `.codex/config.toml`, `AGENTS.md`, hooks を自動ロードする。
