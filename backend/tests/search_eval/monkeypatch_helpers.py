"""
Monkeypatching setup for search evaluation tests.

Extracted from test_live_company_info_search_report.py lines 607-716.
Centralizes all module patches for search test environment.
"""
...
from tests.search_eval.models import HybridRawResult
...
def setup_search_patches(
    ...
    from tests.utils.rate_limiter import rate_limited_request
    ...
    snapshot_db = config.snapshot_db or str(
        backend_root / "tests" / "output" / "ddg_snapshots.db"
    )
    snapshot_cache = None
