...
class TestParentDomainExclusion:
    ...
    def test_parent_company_not_excluded_for_itself(self):
        ...
        assert result is False, "Parent company searching for itself should NOT be excluded"
...
class TestDomainBoundaryValidation:
    ...
    def test_no_partial_match_prefix(self):
        ...
        result = is_parent_domain(
            url="https://smitsui.com/",
            company_name="三井物産スチール"
        )
        assert result is False, "Partial match (prefix) should NOT be detected as parent"

    def test_no_partial_match_suffix(self):
        """部分マッチ（接尾辞）は除外 - permitsui != mitsui"""
        result = is_parent_domain(
            url="https://permitsui.com/",
            company_name="三井物産スチール"
        )
        assert result is False, "Partial match (suffix) should NOT be detected as parent"

    def test_no_partial_match_embedded(self):
        """部分マッチ（埋め込み）は除外 - amitsuit != mitsui"""
        result = is_parent_domain(
            url="https://amitsuit.com/",
            company_name="三井物産スチール"
        )
        assert result is False, "Partial match (embedded) should NOT be detected as parent"

    def test_hyphenated_pattern_prefix(self):
        """ハイフン付きパターン（接頭辞）"""
        result = is_parent_domain(
            url="https://mitsui-career.jp/",
            company_name="三井物産スチール"
        )
        assert result is True, "Hyphenated pattern (prefix) should be detected as parent"

    def test_hyphenated_pattern_suffix(self):
        """ハイフン付きパターン（接尾辞）"""
        result = is_parent_domain(
            url="https://career-mitsui.jp/",
            company_name="三井物産スチール"
        )
        assert result is True, "Hyphenated pattern (suffix) should be detected as parent"

    def test_no_match_similar_company(self):
        """類似企業名は誤マッチしない - sumitomo != mitsui"""
        result = is_parent_domain(
            url="https://sumitomo.com/",
            company_name="三井物産スチール"
        )
    ...
    def test_subsidiary_own_hyphenated_domain_not_parent(self):
        ...
        result = is_parent_domain(
            url="https://www.mitsui-steel.com/recruit/",
            company_name="三井物産スチール"
        )
        assert result is False, "Subsidiary's own hyphenated domain should NOT be parent"
...
class TestWildcardSubsidiaryDetection:
    ...
    def test_nttdata_sbc_detected(self):
        ...
        is_sub, name = is_subsidiary_domain(
            url="https://www.nttdata-sbc.co.jp/recruit/",
            parent_name="NTTデータ"
        )
    ...
    def test_nttdata_force_detected(self):
        ...
        assert name == "NTTデータフォース", f"Expected 'NTTデータフォース', got '{name}'"

    def test_unregistered_nttdata_xxx_detected(self):
        """未登録のnttdata-xxxドメインがワイルドカード検出される"""
        is_sub, name = is_subsidiary_domain(
            url="https://www.nttdata-newsubsidiary.co.jp/",
            parent_name="NTTデータ"
        )
        assert is_sub is True, "nttdata-newsubsidiary should be detected as unregistered subsidiary"
    ...
    def test_nttdata_recruit_official_not_detected(self):
        ...
        assert is_sub is False, "nttdata-recruit.com should NOT be detected as subsidiary"

    def test_nttdata_career_official_not_detected(self):
        """NTTデータキャリアサイト（nttdata-career.com）は子会社として検出されない"""
        is_sub, _ = is_subsidiary_domain(
            url="https://nttdata-career.com/",
            parent_name="NTTデータ"
        )
        assert is_sub is False, "nttdata-career.com should NOT be detected as subsidiary"

    def test_nttdft_detected_as_subsidiary(self):
        """nttdft.com（NTTデータフィナンシャルテクノロジー）が子会社として検出される"""
        is_sub, name = is_subsidiary_domain(
            url="https://www.nttdft.com/recruit/",
            parent_name="NTTデータ"
        )
        assert is_sub is True, "nttdft.com should be detected as subsidiary"
        assert name == "NTTデータフィナンシャルテクノロジー", f"Expected 'NTTデータフィナンシャルテクノロジー', got '{name}'"

    def test_mitsui_unregistered_detected(self):
        """未登録の三井物産関連ドメインがワイルドカード検出される"""
        is_sub, name = is_subsidiary_domain(
            url="https://www.mitsui-newcompany.co.jp/",
            parent_name="三井物産"
        )
        assert is_sub is True, "mitsui-newcompany should be detected as unregistered subsidiary"
        assert "未登録子会社" in name, f"Should indicate unregistered, got '{name}'"
...
class TestParentSearchSubsidiaryDetection:
    ...
    def test_nttdata_detects_mse(self):
        ...
        is_sub, name = is_subsidiary_domain(
            url="https://nttdmse-recruit.snar.jp/",
            parent_name="NTTデータ"
        )
    ...
    def test_nttdata_detects_strategy(self):
        ...
        is_sub, name = is_subsidiary_domain(
            url="https://www.nttdata-strategy.com/recruit/",
            parent_name="NTTデータ"
        )
        assert is_sub is True, "NTTデータ経営研究所 should be detected as subsidiary"
        assert name == "NTTデータ経営研究所", f"Expected 'NTTデータ経営研究所', got '{name}'"
    ...
    def test_nttdata_official_not_detected(self):
        ...
        assert is_sub is False, "NTTデータ official site should NOT be detected as subsidiary"
    ...
    def test_mitsui_official_not_detected(self):
        ...
        assert is_sub is False, "三井物産 official site should NOT be detected as subsidiary"
...
class TestParentCompanyPatternExclusion:
    ...
    def test_nttdata_search_excludes_ntt_pattern(self):
        ...
        assert "ntt" not in patterns, "ntt pattern should NOT be included (parent company pattern)"
