...
Orchestrator that uses the search_eval package to:
1. Load companies
...
All heavy logic has been extracted to backend/tests/search_eval/.
"""
...
async def test_live_company_info_search_report(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # -------------------------------------------------------------------------
    baselines_dir = Path(__file__).resolve().parent / "output" / "baselines"
