...
def _log_openai_empty_json_attempt(
    ...
    if candidates:
        ...
        for i, c in enumerate(candidates[:2]):
            _log(
                feature,
                f"候補{i + 1}メタデータ: {_payload_metadata(c)}",
                WARNING,
            )
    else:
        ...
        _log(
            ...
            f"(output_text: {_payload_metadata(output_text)})",
            WARNING,
        )


def _payload_metadata(value: object) -> str:
    if value is None:
        return "type=NoneType, length=0, sha256=None"
    text = value if isinstance(value, str) else str(value)
    digest = hashlib.sha256(text.encode("utf-8", errors="replace")).hexdigest()
    return f"type={type(value).__name__}, length={len(text)}, sha256={digest[:12]}"
