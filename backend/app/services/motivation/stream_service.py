"""
Motivation SSE stream service.

This module is the CA-1 service-layer home for the motivation
next-question stream use case. The old router module re-exports these
functions temporarily for compatibility with existing imports and tests.
"""
...
def _error_type_from_llm_error(error_type: str | None) -> str:
    if error_type == "parse":
...
def _http_exception_error_payload(exc: HTTPException) -> dict:
    ...
    if isinstance(detail, dict):
        ...
        error_type = detail.get("error_type") or detail.get("code")
        payload = {
            "message": str(message or "AIサービスに接続できませんでした。"),
        ...
        return payload
    return {
        "message": str(detail or "AIサービスに接続できませんでした。"),
        "status_code": exc.status_code,
    }
...
async def _generate_next_question_progress(
    ...
    *,
    ...
    try:
        ...
        yield _sse_event("error", {
            **_http_exception_error_payload(e),
            "internal_telemetry": consume_request_llm_cost_summary("motivation"),
        })
    except Exception:
        yield _sse_event("error", {
