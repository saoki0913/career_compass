...
class ReviewRequest(BaseModel):
    ...
    section_char_limit: Optional[int] = Field(default=None, ge=1, le=1500)
    ...
    @field_validator("content", "section_title")
    @classmethod
    def normalize_and_validate_text(cls, value: str, info) -> str:
        normalized = value.strip()
        if not normalized:
            raise ValueError("must not be blank")
        if info.field_name == "content" and len(normalized) < 6:
            raise ValueError("content must be at least 6 characters")
        return normalized
