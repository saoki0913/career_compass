...
_MOTIVATION_EVALUATION_PROMPT_FALLBACK = """以下の志望動機に関する会話を分析し、4つの要素の充実度を0-100で評価してください。
...
_MOTIVATION_QUESTION_PROMPT_FALLBACK = """あなたは就活生の「志望動機」を深掘りするプロのインタビュアーです。
...
- 必須論点は `industry_reason / company_reason / desired_work / origin_experience / fit_connection / differentiation` の6つ。会話全体では必ず回収すること
- 再深掘りターンでは、前回と同じ言い方・同じ切り口を繰り返さず、別角度で聞き直すこと
- 質問文は命令文ではなく、自然な会話の問いとして書くこと
...
### RAG情報の扱い
- 企業の具体的な情報は、**自然に質問へ入るときだけ** 質問文に織り込むこと
- 無理に企業特徴・職種名・仕事内容を差し込んで、不自然な問いにしないこと
- 質問文に入れない場合でも、`evidence_summary` には参照した根拠を簡潔に残してよい
...
- 入社後にどう価値を出すかまで繋げる
- 十分な品質に達した場合でも、会話を終了させるのではなく「このまま下書きを作れる状態」を前提に、必要なら追加で深掘りできる軽い1問を返す
- 必要に応じて、学生の発言の中の曖昧さや飛躍をやさしく検証する

### 良い質問の例
- `company_reason`: 「東京海上日動のどんな点が、ご自身の就活軸と重なると感じますか？」
- `desired_work`: 「入社後は、どんな相手や課題に向き合う仕事がしたいですか？」
- `fit_connection`: 「その経験のどんな力が、入社後の仕事で活かせそうですか？」

### 悪い質問の例
- `company_reason`: 「この企業のどこに惹かれたかを1文で答える」
- `company_reason`: 「営業 / コンサルティング営業として働くイメージを持てる点が、自分の志向と合うためですか？」
- `desired_work`: 「なぜこの企業に興味を持ったのですか？また、入社後に何をしたいですか？」
...
_MOTIVATION_SUGGESTION_REWRITE_PROMPT_FALLBACK = """あなたは就活生の口調に合わせて、回答候補の文案だけを整える編集者です。
...
2. 質問への**直接の答え**になっている形に整える
3. **事実・固有名詞・数値・経験の内容は追加・削除・変更しない**（言い換えと語順の調整のみ）
4. 候補同士の観点の違いは維持し、似た言い回しに寄せすぎない
5. 新しい企業名・商品名・職種・数字を **絶対に足さない**
6. 元の文が短い場合は、無理に長くしない
7. 出力は **JSONのみ**。説明文やコードブロックは禁止
...
_DRAFT_GENERATION_PROMPT_FALLBACK = """以下の会話内容から、{char_limit}字程度の志望動機ESを作成してください。
