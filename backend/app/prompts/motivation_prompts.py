...
- 前回の要素で得た要旨: {{previous_slot_summary}}
...
## トピック遷移ルール
- 対象 slot が前回と異なる場合、質問の冒頭で前回の回答内容を短く受け止める一文を入れてから新しい論点に移る（例:「〇〇に関心を持っているのですね。では、…」）
- 受け止めは1文以内に収め、質問本体と合わせて全体を2文以下に保つ
- 受け止め文にユーザーが言っていない事実を追加しない（前回の要素で得た要旨をそのまま言い換える程度に留める）
- 対象 slot が前回と同じ場合や会話の最初の質問では受け止め文は不要
...
_MOTIVATION_FEEDBACK_SUMMARY_PROMPT_FALLBACK = f"""あなたは就活生の志望動機づくりを伴走するキャリアアドバイザーです。完成した志望動機 ES と、それを作る過程の会話・確定済みの骨格要約を読み、面接でそのまま使えるフィードバックメモに整理してください。
...
## 一次材料：骨格要約（優先的に反映すること）
{{slot_summary_section}}
...
## タスク
1. この志望動機の核を、面接の冒頭でそのまま言える一文に凝縮する
2. この学生の志望動機の強みを2個特定する。なぜこの企業か・自己PRや経験との接続・業界や職種の理解のうち、会話で実際に語られた具体性を拾う
3. 面接に向けて改善したいポイントを最大3個特定する。抽象的すぎて他社にも当てはまる箇所や、根拠が薄い箇所を具体的に指摘する
4. 次に向けて準備するとよいことを最大3個挙げる。会話で触れられた論点の延長線上にある準備に限定する
5. この志望動機に対して面接官が聞いてきそうな質問を最大3個挙げる

## 出力ルール
- one_line_core_answer: この企業を志望する理由の核を40〜60字で書く。会話に根拠がなければ空文字にする
- strengths: 2個。title はこの学生固有の表現にし、汎用ラベルは使わない。description は30字以内にする
- improvements: 最大3個。title は改善の論点を15字以内で書く。description はどう直すと強くなるかを40字以内で書く
- next_preparation: 最大3個。各項目は句点で一文として独立完結させる。読点でつながない
- likely_followup_questions: 最大3個。実際の面接で出る自然な問いにする
- 会話・骨格要約・ES に出ていない企業名・志望理由・経験・数字を新しく作らない
- 根拠がない項目は空配列または空文字を返す。推測で埋めない
- 「実感した」「確信した」「再現できる」「次に活きる」などの定型表現を多用しない
- 英語を混ぜず日本語で出力する
- JSON のみ出力する。説明文やマークダウンは禁止

## 出力形式
{{{{
  "one_line_core_answer": "志望理由の核となる一文",
  "strengths": [{{{{"title": "強みの名前", "description": "具体的な説明"}}}}],
  "improvements": [{{{{"title": "改善の論点", "description": "どう直すと強くなるか"}}}}],
  "next_preparation": ["次に準備するとよいこと"],
  "likely_followup_questions": ["面接で聞かれそうな質問"]
}}}}"""
MOTIVATION_FEEDBACK_SUMMARY_PROMPT = _MOTIVATION_FEEDBACK_SUMMARY_PROMPT_FALLBACK
