...
STYLE_RULES: list[StyleRule] = [
    ...
    StyleRule(
        "複数の施策がある場合は①②を文中にインラインで置き、各施策を「では」等の助詞で地の文につなげる（リスト化しない）",
        "all",
        frozenset({"gakuchika"}),
        priority="should",
    ),
...
TEMPLATE_GUIDANCE: dict[str, list[str]] = {
    ...
    "post_join_goals": [
        ...
        "入社後の目標が複数ある場合は「やりたいことは2つある。1つ目は〜2つ目は〜」で数を宣言し、各目標を「目標→原体験→企業での実現方法」で完結させる",
    ],
    "role_course_reason": [
        "冒頭でその職種・コースの魅力を具体的に述べる",
        "経験を述べる文では、何を・どう行い・どんな結果につながったかを含める",
        "企業固有の職種特性・制度に触れる場合は、自分の適性と接続する",
        "選択理由が複数ある場合は「理由は二つある。第一に〜第二に〜」で数を宣言し、各理由を「職種の魅力→適性の根拠→将来の貢献」で完結させる",
    ],
    "self_pr": [
