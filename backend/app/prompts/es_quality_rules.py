...
STYLE_RULES: list[StyleRule] = [
    ...
    StyleRule(
        "複数の施策がある場合は①②を文中にインラインで置く（リスト化しない）。簡潔な列挙（「①XX②YYの2施策」）は1文内でよいが、各施策を説明するときは①の直後に行動を続けて各項目を完結した文にする（句点「。」で区切る）",
        "all",
        frozenset({"gakuchika"}),
        priority="should",
    ),
...
TEMPLATE_GUIDANCE: dict[str, list[str]] = {
    ...
    "gakuchika": [
        ...
        "複数の施策がある場合は①②または「第一に/第二に」を文中にインラインで使う。簡潔な列挙は1文内（「①XX②YYの2施策を実施した」）でよいが、各施策を説明するときは①の直後に行動を続けて各項目を完結した文にする（悪い例:「①XXでは…し、②YYでは…した。」→ 正しい例: 宣言「そこで2つの施策を実施した。」+展開「①美容学校に協賛を交渉し…実現した。②学食の社長に…合意を取り付けた。」）。「一つ目は〜二つ目は〜」のようなリスト風の羅列にしない",
    ],
    "intern_reason": [
