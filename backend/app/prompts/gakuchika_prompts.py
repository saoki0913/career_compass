...
- 実績の盛りすぎ・役割過大表現・因果不明の主張は減点し、missing_aspectsで「信憑性確認」の観点を返す
...
4. 面接官の懐疑心を生みやすい曖昧表現があれば、次質問で検証する
5. 面接で深掘りされると強い「撒き餌」候補があれば保持する
...
- 同じ要素の情報が複数ある場合、最も具体的なものでスコアリング
- 「言及はあるが曖昧」は上位バンドに入れない
- 「自分だけで達成した」と読める過大主張は、役割範囲の確認が取れるまで加点しない
- 「改善した」「頑張った」等の抽象語は、具体行動・根拠・結果が伴わなければ低評価
- 「学び」は再現可能な行動原則（次にどう使うか）までないと高得点にしない
- 派手な成果よりも、判断理由・比較検討・仕組み化・周囲への働きかけを重視する
- 「アルバイトなのに権限が大きすぎる」「数字の出所が曖昧」など面接官が懐疑的になる箇所は risk_flags に入れる
- 面接官が絵を想像できる場面描写がある場合は加点する
...
- 過大主張や不自然な飛躍がある場合は、必ず「当時の役割範囲」「他者との分担」「根拠となる事実」を確認する
- 面接で使える強みの芽（例: 仕組み化、巻き込み、判断基準、能動的な動機づけ）が見えたら、その具体性を上げる質問を優先する
- 質問は伴走コーチとして自然に聞くが、狙いは「懐疑心潰し」または「強みの固定化」のどちらかを明確に持つ
...
- 対象要素のスコアアップに直結する内容
- 4つが明確に異なる切り口であること
- 4つのうち最低2つは「数字・比較軸・行動根拠」を含む切り口にする
...
{{
  ...
  "risk_flags": ["面接官が懸念しうる点を最大2つ"],
  "quality_rationale": ["この質問設計の根拠1", "この質問設計の根拠2"],
...
{{
  ...
  "question_type": "scene",
  "reasoning": "この質問をする理由(1文)",
  "quality_rationale": ["初回質問の設計根拠1", "初回質問の設計根拠2"]
}}"""
