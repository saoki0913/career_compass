...
def _get_source_type(url: str, company_name: str) -> str:
    ...
    Registered ownership is the source of truth. Job-site/blog detection is
    layered on top, but related-company domains are never promoted to official.

    Returns: "official" | "parent" | "subsidiary" | "job_site" | "blog" | "other"
    """
...
def _detect_other_graduation_years(
    ...
    # Recruitment context guard: avoid treating generic page dates as grad years.
    ...
    # Patterns to detect grad/fiscal year expressions.
    patterns = [
        r"(20\d{2})\s*卒",                 # 2027卒
        r"(?<!\d)(\d{2})\s*卒",            # 27卒
        r"(20\d{2})\s*年度",               # 2027年度
        r"(?:fy|fiscal)\s*[-/]?\s*(20\d{2})",  # FY2027 / fiscal 2027
        r"(?:fy|fiscal)\s*[-/]?\s*(\d{2})",    # FY27
...
class UploadCorporatePdfResponse(BaseModel):
    """Response from uploaded corporate PDF ingestion."""
...
def _should_run_high_accuracy_pdf_ocr(
    *,
    billing_plan: str,
    content_type: str | None,
    page_count: int | None,
    google_result_text: str,
    google_quality_score: float | None,
) -> bool:
    if billing_plan not in {"standard", "pro"}:
        return False
    if content_type not in {"ir_materials", "midterm_plan"}:
        return False
    if (page_count or 0) < int(settings.pdf_ocr_high_accuracy_min_pages):
        return False
    if not (google_result_text or "").strip():
        return True
    if (
        google_quality_score is not None
        and google_quality_score < float(settings.pdf_ocr_google_weak_quality_score)
    ):
        return True
    return _chars_per_page(google_result_text, page_count) < float(
        settings.pdf_ocr_google_weak_chars_per_page
    )
