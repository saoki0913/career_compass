...
**code-reviewer は blocking reviewer** として扱う。code-reviewer を起動した場合、その結果が返るまでは実装判断の確定、計画の確定、commit / push、最終回答へ進まない。`/codex:status` や `wait_agent` が timeout しただけでは承認扱いにしない。明示的にユーザーが「code-reviewer を待たずに続行」と指示した場合のみ例外とし、その事実を最終回答に記録する。
...
4. レビュー結果の確認:
   ...
   - **code-reviewer の完了は必須**。他 agent のレビューが揃っていても、code-reviewer が pending / running / timeout のままなら Section B-2、commit、push、最終回答に進まない。
5. Status 判定:
   - レビュー失敗 / TIMEOUT / CODEX_ERROR / PARSE_FAILURE → Claude 自身の code-reviewer skill で fallback review
   - APPROVE → commit を続行
   - REQUEST_CHANGES かつ maxSeverity == high → 指摘を修正してから commit
...
5. [Plugin] /codex:result で全結果収集（code-reviewer を含む場合は必ず完了まで待つ）
6. [Claude] 結果統合・競合解決
...
- code-reviewer 結果を取得するまで次工程へ進まない。pending / running / timeout は approve ではない。
