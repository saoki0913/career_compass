...
const file = path.join(project, "docs/plan/maintainability-clean-architecture-tasks.json");

if (!taskId || !STATUS_VALUES.has(status)) {
  process.stderr.write(
    "Usage: node scripts/plan/update-maintainability-task-status.mjs --id <task-id> --status <Todo|Doing|Blocked|Review|Done> [--evidence <text>] [--notes <text>]\n",
  );
  process.exit(2);
}
